...
export default function Home() {
  ...
  function HomePage() {
    ...
    return <>
      <section className="dashboard-kicker"><span>{currentPeriod.replace("-", " 年 ")} 月 · {template.label}经营</span><h1>{template.storeName}，这月赚得怎么样？</h1><em>已按真实交易核算收入、成本与经营费用</em></section>
      <OperatingSnapshot revenue={totals.revenue} cost={totals.totalCost} profit={totals.operatingProfit} />
      <button className="budget-summary" onClick={() => goSub("budget")}><span>预算剩余</span><b>{yuan(totals.budgetRemaining)}</b><em>{totals.budgetUsed >= 100 ? "已超预算" : `${totals.budgetUsed.toFixed(1)}% 已用`}</em><ChevronRight size={16} /></button>
      {book.state.workspace.dataMode === "legacy_review" && totals.grossSales === 0 && <button className="judgment-row warning" onClick={openNewRecord}><span>＋</span><div><b>补录销售收入，完成本期核算</b><em>历史模板营收未自动迁入，避免重复确认。</em></div><ChevronRight size={18} /></button>}
      {!(book.state.workspace.dataMode === "legacy_review" && totals.grossSales === 0) && <button className="judgment-row" onClick={() => setTab("analysis")}><span>＝</span><div><b>{template.risk}</b><em>{template.riskNote}</em></div><ChevronRight size={18} /></button>}
  ...
  function OrdersPage() {
    ...
    return <><section className="sub-intro compact"><span>{template.label} · 订单经营</span><h1>订单</h1></section><div className="list-toolbox"><label className="month-filter"><CalendarDays size={16} /><select aria-label="筛选订单月份" value={orderMonth} onChange={(event) => setOrderMonth(event.target.value)}><option value="all">全部月份</option>{orderMonths.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select></label><label className="search-field compact-search"><Search size={16} /><input value={orderSearch} onChange={(event) => setOrderSearch(event.target.value)} placeholder="搜索订单号、客户或 SKU" /></label></div><div className="order-filter-chips"><button className={orderStatusFilter === "all" ? "active" : ""} onClick={() => setOrderStatusFilter("all")}>全部</button><button className={orderStatusFilter === "low_profit" ? "active" : ""} onClick={() => setOrderStatusFilter("low_profit")}>低利润</button><button className={orderStatusFilter === "refund" ? "active" : ""} onClick={() => setOrderStatusFilter("refund")}>退款</button></div><div className="result-summary">{hasFilter ? `已找到 ${filteredOrders.length} 笔订单` : `共 ${orders.length} 笔订单`}</div><div className="order-list">{filteredOrders.map((order) => { const revenue = order.lines.reduce((sum, line) => sum + line.unitPriceFen * line.quantity / 100, 0); const cogs = order.lines.reduce((sum, line) => sum + line.unitCostFen * line.quantity / 100, 0); const fulfillment = order.lines.reduce((sum, line) => sum + line.quantity, 0) * order.pricing.fulfillmentCost; const contribution = revenue * (1 - order.pricing.commissionRatePct / 100) - cogs - fulfillment; const warning = orderWarnings.find((item) => item.orderId === order.id); return <button key={order.id} onClick={() => openOrder(order.id)}><div><b><Highlight value={order.orderNo || "未编号订单"} query={orderSearch} /></b><em><Highlight value={`${order.occurredAt} · ${channelLabel[order.channel]} · ${order.buyer || "散客"}`} query={orderSearch} /></em><small><Highlight value={order.lines.map((line) => line.skuName).join(" / ")} query={orderSearch} /></small></div><strong>{yuan(revenue)}<em className={warning ? "attention" : ""}>{warning ? (warning.type === "below_break_even" ? "低于保本" : "低于目标") : `贡献利润 ${yuan(contribution)}`}</em></strong><ChevronRight size={16} /></button>; })}{!filteredOrders.length && <div className="empty-state business-empty">{hasFilter ? <><b>没有匹配结果</b><p>{`“${orderSearch || monthLabel(orderMonth)}”下暂无订单，试试更换月份或关键词。`}</p></> : <><span>＋</span><b>还没有订单</b><p>记录第一笔订单，开始算清这笔生意的收入、成本和利润。</p><button onClick={openNewOrder}>＋ 记录第一笔订单</button></>}</div>}</div><section className="after-sale-status"><span>售后处理</span><b>{periodRefunds.length ? `本期 ${periodRefunds.length} 笔退款` : "本期无退款"}</b></section>{refundPareto.length >= 2 && <section className="order-refund-chart"><div className="section-title"><div><span>售后原因</span><h2>退款占比</h2></div></div><RefundPareto items={refundPareto} onSelect={(reason) => { setOrderStatusFilter("refund"); setOrderSearch(reason); }} /></section>}<button className="fixed-primary list-primary" onClick={openNewOrder}><Plus size={18} />记录订单</button></>;
  }
  ...
  function AnalysisPage() {
    ...
    const primaryReason = periodCategories[0];
    return <><section className="screen-title"><span>经营分析 · {period.replace("-", " 年 ")} 月</span><h1>利润与成本</h1></section><div className="segment-control"><button className={analysisPeriod === "current" ? "active" : ""} aria-pressed={analysisPeriod === "current"} onClick={() => setAnalysisPeriod("current")}>本月</button><button className={analysisPeriod === "last" ? "active" : ""} aria-pressed={analysisPeriod === "last"} onClick={() => setAnalysisPeriod("last")}>上月</button></div>{hasRevenue ? <ProfitWaterfall revenue={analysisRevenue} cogs={analysisCogs} expenses={analysisExpenses} profit={periodView.totals.operatingProfit} onSelect={(key) => { if (key === "cogs" || key === "expenses") { setRecordFilter("expense"); setRecordMonth(period); goSub("records"); } else if (key === "revenue") { setRecordFilter("income"); setRecordMonth(period); goSub("records"); } else goSub("reports"); }} /> : <button className="analysis-no-revenue" onClick={openNewRecord}><span>本期尚未录入销售收入</span><b>补录收入后生成利润瀑布图</b><ChevronRight size={16} /></button>}<section className="analysis-judgment"><span>第一成本</span><b>{primaryReason ? `${primaryReason.label} ${Math.round(primaryReason.amount / Math.max(periodCost, 1) * 100)}%` : "等待更多成本数据"}</b><button onClick={() => primaryReason ? (setRecordSearch(primaryReason.label), setRecordMonth(period), goSub("records")) : goSub("record")}>{primaryReason ? `复核${primaryReason.label}` : "＋ 记录第一笔成本"}<ChevronRight size={15} /></button></section>
      ...
      <section className="section-block"><div className="section-title"><div><span>成本构成</span><h2>金额与环比</h2></div></div><div className="ranking-card">{periodCategories.map((item, index) => { const delta = categoryDeltaByKey.get(item.key); return <button key={item.key} onClick={() => { setRecordSearch(item.label); setRecordMonth(period); goSub("records"); }}><span className="rank">{String(index + 1).padStart(2, "0")}</span><span className="rank-name"><i style={{ background: item.color }} />{item.label}</span><span className="rank-bar"><b style={{ width: `${item.amount / Math.max(periodCategories[0]?.amount || 1, 1) * 100}%`, background: item.color }} /></span><strong>{yuan(item.amount)}<small className={(delta?.delta || 0) > 0 ? "up" : "down"}>{delta?.deltaRate === null ? "新发生" : `${(delta?.delta || 0) >= 0 ? "↑" : "↓"}${Math.abs(delta?.deltaRate || 0)}%`}</small></strong></button>; })}</div></section>
      ...
      <section className="section-block"><div className="section-title"><div><span>订单核算</span><h2>SKU 销售与退款</h2></div><button onClick={() => goSub("orders")}>订单账本 <ChevronRight size={15} /></button></div><div className="supplier-rank"><button onClick={() => goSub("orders")}><span>单</span><b>已录订单</b><em>当前行业订单</em><strong>{orders.length} 笔</strong></button><button onClick={() => goSub("orders")}><span>退</span><b>退款案例</b><em>已关联 SKU 与原因</em><strong>{refunds.length} 笔</strong></button></div></section><section className="section-block"><div className="section-title"><div><span>定价预警</span><h2>需要复核的订单</h2></div><button onClick={() => goSub("orders")}>全部 <ChevronRight size={15} /></button></div><div className="pricing-warning-list">{orderWarnings.slice(0, 3).map((warning) => <button key={warning.id} onClick={() => openOrder(warning.orderId)}><span className={warning.type === "below_break_even" ? "danger" : "warning"}><CircleAlert size={17} /></span><div><b>{warning.type === "below_break_even" ? "低于保本价" : "低于目标毛利"} · {warning.orderNo}</b><em>{warning.type === "below_break_even" ? `实收 ${yuan(warning.revenue)}，保本线 ${yuan(warning.breakEvenRevenue)}` : `贡献毛利 ${warning.contributionMarginRate}% / 目标 ${warning.targetMarginRate}%`}</em></div><strong>{warning.type === "below_break_even" ? `-${yuan(Math.abs(warning.contribution))}` : `${warning.contributionMarginRate}%`}</strong><ChevronRight size={15} /></button>)}{!orderWarnings.length && <div className="empty-state">暂无低于保本价或目标毛利率的订单。</div>}</div></section>
      <section className="section-block"><div className="section-title"><div><span>订单售后</span><h2>退款原因</h2></div><button onClick={() => { setRecordFilter("refund"); setRecordMonth(period); goSub("records"); }}>流水 <ChevronRight size={15} /></button></div><RefundPareto items={refundPareto} onSelect={(reason) => { setRecordFilter("refund"); setRecordMonth(period); setRecordSearch(reason); goSub("records"); }} /></section>
      <section className="section-block"><div className="section-title"><div><span>订单 SKU</span><h2>商品真实毛利</h2></div><button onClick={() => goSub("skus")}>全部 <ChevronRight size={15} /></button></div><div className="top-card-list">{skuMetrics.slice(0, 3).map((sku, index) => <button key={sku.id} onClick={() => goSub("skus")}><span>{index + 1}</span><b>{sku.name}</b><em>售 {sku.soldQuantity}{sku.unit} · 退 {sku.refundedQuantity}{sku.unit}</em><strong>{sku.grossMarginRate}% 毛利</strong></button>)}{!skuMetrics.length && <div className="empty-state">创建订单后，这里会展示每个 SKU 的销量、退款与真实毛利。</div>}</div></section>
  ...
  function CardsPage() {
    ...
    return <><section className="sub-intro compact"><span>{template.label} · {template.entityLabel}核算</span><h1>{template.entityLabel}成本</h1></section><label className="search-field"><Search size={16} /><input value={cardSearch} onChange={(event) => setCardSearch(event.target.value)} placeholder={`搜索${template.entityLabel}名称或类型`} /></label><div className="product-cost-list">{visibleCards.map((card) => { const value = calcCard(card); const maxHistory = Math.max(...card.history, value.cost, 1); const unitProfit = card.salePrice - value.cost; return <button key={card.id} onClick={() => openCard(card.id)}><div className="product-card-head"><span><PackageOpen size={19} /></span><div><b>{card.name}</b><em>{card.kind || template.entityLabel} · {card.unit}</em></div><i className={card.status === "risk" ? "risk" : card.status === "attention" ? "attention" : "healthy"}>{card.status === "risk" ? "需复核" : card.status === "attention" ? "成本上升" : "稳定"}</i></div><div className="product-kpis"><label><em>单位成本</em><b>{yuan(value.cost)}</b></label><label><em>售价</em><b>{card.salePrice > 0 ? yuan(card.salePrice) : "未填写"}</b></label><label><em>单件利润</em><b className={unitProfit < 0 ? "negative" : ""}>{card.salePrice > 0 ? yuan(unitProfit) : "—"}</b></label></div><div className="product-microtrend">{card.history.map((amount, index) => <span key={`${amount}-${index}`}><i style={{ height: `${Math.max(8, amount / maxHistory * 100)}%` }} /><em>{index + 2}月</em></span>)}</div><div className="product-actions"><span>成本构成 {card.items.length}项</span><span>智能定价 <ChevronRight size={14} /></span></div></button>; })}{!visibleCards.length && <div className="empty-state">没有匹配的成本卡。</div>}</div><button className="fixed-primary list-primary" onClick={openNewCard}><Plus size={18} />新增{template.entityLabel}成本卡</button></>;
  }
  ...
  function BudgetPage() {
    ...
    return <><section className="screen-title budget-title"><span>{template.label} · {currentPeriod.replace("-", " 年 ")} 月</span><h1>预算管理</h1></section><BudgetRing burn={budgetBurn} onClick={() => notify("已按当前入账成本刷新预算预测")} /><section className="budget-forecast"><span>月末预计趋势</span><div><i /><i /><i /></div><b>{budgetBurn.state === "over" ? `已超预算 ${yuan(Math.abs(budgetBurn.remaining))}` : budgetBurn.state === "risk" ? `月末预计 ${yuan(budgetBurn.forecast)}` : `预计结余 ${yuan(Math.max(0, budgetBurn.budget - budgetBurn.forecast))}`}</b></section><form className="record-form compact-form" onSubmit={saveBudget}><label>月度预算<div className="amount-input"><span>¥</span><input name="budget" type="number" min="1" defaultValue={totals.budget} /></div></label><button className="fixed-primary form-save" type="submit">调整预算</button></form></>;
  }
