...
export default function Home() {
  ...
  function PricingPage() {
    ...
    const switchPricingChannel = (channel: OrderChannel) => { const config = channelTemplates[channel]; setPricingChannel(channel); setPricingPlatformRate(config.commissionRatePct); setPricingFulfillmentCost(config.fulfillmentCost); setPricingTargetMargin(config.targetContributionMarginPct); };
    ...
    return <>
      ...
      {suggested?.available ? <section className="pricing-recommend"><span>建议售价 · 目标贡献毛利 {targetMargin}%</span><strong>{yuan(suggested.price)}</strong><p>原始测算 {yuan(suggested.rawPrice)}，直接保留两位小数；每{activeCard.unit}贡献 {yuan(suggested.contributionPerUnit)}，实际贡献毛利 {suggested.contributionMarginPct}% 。</p><button onClick={() => writePrice(suggested.price)}><Calculator size={17} />一键写入售价</button></section> : <div className="pricing-error"><CircleAlert size={18} />{pricingInputsReady ? suggested?.reason : "请补齐渠道费率、履约费用和目标毛利率"}</div>}
