...
export default function Home() {
  ...
  function AnalysisPage() {
    ...
    const supplierRankings = buildSupplierCostRankings({ entries: book.state.entries, suppliers, industryId: book.activeIndustryId, period });
    ...
    return <><AnalysisPageContent /><section className="analysis-supplement ledger-page-shell" aria-label="成本对照与供应商排行"><section className="analysis-structure-compare ledger-surface"><div className="analysis-card-head"><h2>本期 / 上期结构</h2><span>正向成本占比</span></div>{structureComparison.length ? <div className="analysis-compare-list">{structureComparison.map((item) => <button key={item.key} onClick={() => { setRecordSearch(item.label); setRecordMonth(period); goSub("records"); }}><span><b>{item.label}</b><em>本期 {item.share}% · 上期 {item.previousShare}%</em></span><strong className={item.shareDelta > 0 ? "up" : item.shareDelta < 0 ? "down" : "flat"}>{item.shareDelta > 0 ? "+" : ""}{item.shareDelta}pt</strong><ChevronRight size={15} /></button>)}</div> : <div className="analysis-empty-copy">本期与上期均暂无可对照的正向成本。</div>}</section><section className="analysis-supplier-rank ledger-surface"><div className="analysis-card-head"><h2>供应商成本排行</h2><span>已关联支出</span></div>{supplierRankings.length ? <div className="analysis-supplier-list">{supplierRankings.map((item, index) => <button key={item.supplierId} onClick={() => openSupplierRecords(item.supplierId)}><i>{index + 1}</i><span><b>{item.label}</b><em>{item.entryCount} 笔已入账支出 · 占已关联成本 {item.share}%</em></span><strong>{yuan(item.amount)}</strong><ChevronRight size={15} /></button>)}</div> : <button className="analysis-supplier-empty" onClick={() => { setRecordType("expense"); openNewRecord(); }}><span><b>暂无已关联供应商成本</b><em>记录支出时关联供应商，即可在此查看排行。</em></span><ChevronRight size={16} /></button>}</section></section></>;
  }
