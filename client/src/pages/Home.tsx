...
import { buildBudgetBurn, buildCategoryDeltas, buildProfitBridge, buildRefundPareto } from "@/lib/chart-metrics";
...
function ProfitBridge({ items, onSelect }: { items: ReturnType<typeof buildProfitBridge>; onSelect?: (key: string) => void }) {
  const maximum = Math.max(...items.map((item) => Math.abs(item.amount)), 1);
  return <section className="profit-bridge"><div className="chart-heading"><span>本期利润桥</span><b>{items.at(-1)?.label} {yuan(Math.abs(items.at(-1)?.amount || 0))}</b></div><div>{items.map((item) => <button key={item.key} className={item.role} onClick={() => onSelect?.(item.key)}><span>{item.label}</span><i><em style={{ width: `${Math.max(8, Math.abs(item.amount) / maximum * 100)}%` }} /></i><strong>{item.amount < 0 ? "−" : ""}{yuan(Math.abs(item.amount))}</strong></button>)}</div></section>;
}
function BudgetBurn({ burn, onClick }: { burn: ReturnType<typeof buildBudgetBurn>; onClick: () => void }) {
  const projected = Math.min(100, Math.max(0, burn.forecastRate));
  return <button className={`budget-burn ${burn.state}`} onClick={onClick}><div><span>预算燃尽</span><b>{burn.usedRate}% 已用</b></div><i><em style={{ width: `${Math.min(100, burn.usedRate)}%` }} /><small style={{ left: `${projected}%` }} /></i><div><strong>{yuan(burn.used)} / {yuan(burn.budget)}</strong><em>{burn.state === "over" ? "已超预算" : burn.state === "risk" ? `月末预计 ${yuan(burn.forecast)}` : `剩余 ${yuan(burn.remaining)}`}</em></div></button>;
}
...
export default function Home() {
  ...
  const dashboardBridge = buildProfitBridge({ netRevenue: totals.revenue, cogs: totals.cogs, operatingExpense: totals.operatingExpense, operatingProfit: totals.operatingProfit });
  const dashboardBurn = buildBudgetBurn({ budget: totals.budget, used: totals.totalCost, dayOfMonth: currentDay, daysInMonth: new Date(periodYear, periodMonth, 0).getDate() });
  ...
  function HomePage() {
    ...
    return <>
      ...
      <ProfitBridge items={dashboardBridge} onSelect={(key) => { if (key === "revenue") { setRecordFilter("income"); setRecordMonth(currentPeriod); goSub("records"); } else if (key === "cogs" || key === "opex") setTab("analysis"); }} />
      <BudgetBurn burn={dashboardBurn} onClick={() => goSub("budget")} />
  ...
  function AnalysisPage() {
    ...
    const analysisBridge = buildProfitBridge({ netRevenue: periodView.totals.revenue, cogs: periodView.totals.revenue - periodView.totals.grossProfit, operatingExpense: periodView.totals.grossProfit - periodView.totals.operatingProfit, operatingProfit: periodView.totals.operatingProfit });
    const periodDate = period.split("-").map(Number);
    const periodDays = new Date(periodDate[0], periodDate[1], 0).getDate();
    const analysisBurn = buildBudgetBurn({ budget: totals.budget, used: periodCost, dayOfMonth: period === currentPeriod ? currentDay : periodDays, daysInMonth: periodDays });
    ...
    return <><section className="screen-title"><span>经营分析 · {period.replace("-", " 年 ")} 月</span><h1>利润与成本</h1><p>仅展示可行动的期间数据。</p></section><div className="segment-control"><button className={analysisPeriod === "current" ? "active" : ""} aria-pressed={analysisPeriod === "current"} onClick={() => setAnalysisPeriod("current")}>本月</button><button className={analysisPeriod === "last" ? "active" : ""} aria-pressed={analysisPeriod === "last"} onClick={() => setAnalysisPeriod("last")}>上月</button></div><ProfitBridge items={analysisBridge} onSelect={(key) => { if (key === "cogs" || key === "opex") { setRecordFilter("expense"); setRecordMonth(period); goSub("records"); } }} /><BudgetBurn burn={analysisBurn} onClick={() => goSub("budget")} /><section className="analysis-judgment"><span>第一成本</span><b>{primaryReason ? `${primaryReason.label} ${Math.round(primaryReason.amount / Math.max(periodCost, 1) * 100)}%` : "等待更多成本数据"}</b><button onClick={() => primaryReason ? (setRecordSearch(primaryReason.label), setRecordMonth(period), goSub("records")) : goSub("record")}>{primaryReason ? `复核${primaryReason.label}` : "＋ 记录第一笔成本"}<ChevronRight size={15} /></button></section>
