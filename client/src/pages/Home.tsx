...
function OperatingSnapshot({ decision, priority, range, hasData, costRate, profitMarginRate, profitDelta, onSelectRange, onOpenPriority }: { decision: HomeDecision; priority: HomeDecisionNotification | null; range: HomeTimeRangeWindow; hasData: boolean; costRate: number | null; profitMarginRate: number | null; profitDelta: number; onSelectRange: (range: HomeTimeRange) => void; onOpenPriority: (priority: HomeDecisionNotification) => void }) {
  ...
  const monthlyPriority = priority && <button className={`home-decision-risk${hasData ? "" : " home-decision-monthly-risk"}`} data-tone={priority.tone} onClick={() => onOpenPriority(priority)}><CircleAlert size={16} aria-hidden="true" /><span><em>本月待办</em><b>{priority.title}</b></span><small>{priority.action}</small><ChevronRight size={17} aria-hidden="true" /></button>;
...
function ProfitTrend({ items, onOpen }: { items: { month: string; revenue: number; cost: number }[]; onOpen: (month: string) => void }) {
...
export default function Home() {
  ...
  function HomePage() {
    ...
    return <div className="prototype-home home-redesign" data-chart-role={chartRole("home").primary}>
      <section className="dashboard-kicker home-context home-identity-context"><span><i><IndustryIcon size={15} aria-hidden="true" /></i><b>{template.storeName}</b><em>{homeDecision.context.industryLabel} · {homeDecision.context.period.replace("-", " 年 ")} 月</em></span><button onClick={() => goSub("notifications")}>查看本月提醒 <ChevronRight size={14} /></button></section>
      <OperatingSnapshot decision={homeRangeDecision} priority={homeDecision.priority} range={homeRange} hasData={homeRangeHasData} costRate={costRate} profitMarginRate={profitMarginRate} profitDelta={homeRangeProfitDelta} onSelectRange={setHomeTimeRange} onOpenPriority={openHomeDecision} />
  ...
  function AnalysisPageContent() {
    ...
    return <div className="prototype-analysis ledger-page-shell analysis-diagnosis-page analysis-profit-page" data-chart-role={chartRole("analysis").primary}>
      <section className="prototype-analysis-title ledger-page-heading"><div><h1>经营分析 · {period.replace("-", " 年 ")} 月</h1><p>先看利润，再定位成本和风险。</p></div><div className="segment-control" aria-label="分析期间"><button className={analysisPeriod === "current" ? "active" : ""} onClick={() => setAnalysisPeriod("current")}>本月</button><button className={analysisPeriod === "last" ? "active" : ""} onClick={() => setAnalysisPeriod("last")}>上月</button></div></section>
