...
function ListRowMoreMenu({ id, editLabel, deleteLabel, onEdit, onDelete }: { id: string; editLabel: string; deleteLabel: string; onEdit: () => void; onDelete: () => void }) {
  ...
  return <span className="row-more" data-testid={`row-more-${id}`}><button type="button" className="more-trigger row-more-trigger" aria-expanded={open} aria-controls={`row-more-${id}`} aria-label="更多操作" onClick={(event) => { event.stopPropagation(); setOpen((value) => !value); }}><MoreHorizontal size={16} strokeWidth={1.5} /></button>{open && <div className="detail-more-menu" id={`row-more-${id}`} onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => { setOpen(false); onEdit(); }}><Pencil size={16} strokeWidth={1.5} />{editLabel}</button><button type="button" className="danger" onClick={() => { setOpen(false); onDelete(); }}><Trash2 size={16} strokeWidth={1.5} />{deleteLabel}</button></div>}</span>;
}
...
function HomeRecentActivity({ items, onOpenRecord, onOpenOrder, onOpenAll }: { items: HomeRecentActivityItem[]; onOpenRecord: (id: string) => void; onOpenOrder: (id: string) => void; onOpenAll: () => void }) {
  return <section className="home-recent-activity" aria-labelledby="home-recent-activity-title" data-testid="home-recent-activity">
    <div className="home-recent-activity-head"><span><em>最近动态</em><b id="home-recent-activity-title">最新入账与订单</b></span><button type="button" onClick={onOpenAll}>全部流水 <ChevronRight size={16} strokeWidth={1.5} /></button></div>
    {items.length ? <div className="home-recent-activity-list">{items.map((item) => <button key={item.id} type="button" onClick={() => item.source === "order" ? onOpenOrder(item.sourceId) : onOpenRecord(item.sourceId)}><i className={item.tone}>{item.tone === "order" ? <ClipboardList size={18} strokeWidth={1.5} /> : <ReceiptText size={18} strokeWidth={1.5} />}</i><span><b>{item.title}</b><em>{item.detail} · {item.date.replaceAll("-", "/")}</em></span><strong className={item.tone === "income" || item.tone === "order" ? "income" : "expense"}>{item.tone === "income" || item.tone === "order" ? "+" : "−"}{yuan(item.amount)}</strong><ChevronRight size={18} strokeWidth={1.5} /></button>)}</div> : <HomeChartEmpty title="暂无经营动态" copy="从记一笔或记录订单开始，最新变动会出现在这里" action="记一笔" onClick={onOpenAll} />}
  </section>;
}
...
function HomeChartEmpty({ title, copy, action, onClick }: { title: string; copy: string; action: string; onClick: () => void }) {
  return <button className="home-chart-empty" onClick={onClick}><span>{title}</span><b>{copy}</b><small>{action}<ChevronRight size={16} strokeWidth={1.5} /></small></button>;
}
...
function AnalysisControlHub({ progress, reports, onOpenTargets, onOpenIndirectCosts, onOpenReport, onOpenAllReports }: { progress: ReturnType<typeof buildSalesTargetProgress>; reports: Report[]; onOpenTargets: () => void; onOpenIndirectCosts: () => void; onOpenReport: (id: string) => void; onOpenAllReports: () => void }) {
  ...
  return <section className="analysis-control-hub ledger-page-shell" aria-label="经营控制与经营报表">
    <section className="analysis-control-surface ledger-surface"><div className="analysis-card-head"><div><h2>经营控制</h2><span>本月实时目标与费用设置</span></div></div><div className="analysis-control-actions"><button type="button" onClick={onOpenTargets}><span><em>月销售目标</em><b>{targetSummary.primary}</b><small>{targetSummary.detail}</small></span><ChevronRight size={18} strokeWidth={1.5} /></button><button type="button" onClick={onOpenIndirectCosts}><span><em>间接费用项</em><b>费用归集与分摊</b><small>房租、水电、基础人工等经营费用</small></span><ChevronRight size={18} strokeWidth={1.5} /></button></div></section>
    <section className="analysis-report-hub ledger-surface"><div className="analysis-card-head"><div><h2>经营报表</h2><span>正式月度报表，可封存与导出</span></div><button type="button" onClick={onOpenAllReports}>全部 <ChevronRight size={16} strokeWidth={1.5} /></button></div>{latestReport ? <button className="analysis-report-latest" type="button" onClick={() => onOpenReport(latestReport.id)}><span><FileText size={18} strokeWidth={1.5} /><i><b>{latestReport.month.replace("-", " 年 ")} 月经营报表</b><em>{latestReport.status === "closed" ? "已封存快照" : "实时草稿 · 可封存"}</em></i></span><strong>{latestReport.grossMarginRate}%<small>毛利率</small></strong><ChevronRight size={18} strokeWidth={1.5} /></button> : <div className="analysis-report-empty">暂无月度经营报表；录入流水后将按当月口径生成实时草稿。</div>}</section>
  </section>;
}
...
function SalesTargetProgress({ progress, runRateForecast, editing, input, onInputChange, onEdit, onSave, onCancel, onOpenOrders }: { progress: ReturnType<typeof buildSalesTargetProgress>; runRateForecast: number; editing: boolean; input: string; onInputChange: (value: string) => void; onEdit: () => void; onSave: (event: FormEvent<HTMLFormElement>) => void; onCancel: () => void; onOpenOrders: () => void }) {
  ...
  return <section className={`home-chart-card home-sales-target template-bullet-goal ${progress.state}`} data-chart-template="bullet-goal"><div className="home-chart-head"><span>本月销售目标</span><button onClick={onEdit}>调整目标 <Pencil size={16} strokeWidth={1.5} /></button></div><div className="sales-target-body"><div className="sales-target-ring" role="img" aria-label={ringSummary}><svg viewBox="0 0 120 120" aria-hidden="true" focusable="false"><circle className="sales-target-ring-track" cx="60" cy="60" r={ringRadius} /><circle className="sales-target-ring-progress" cx="60" cy="60" r={ringRadius} strokeDasharray={ringCircumference} strokeDashoffset={ringOffset} /></svg><span><em>完成率</em><b>{progress.completionRate}%</b><small title={yuan(progress.revenue)}>{formatMoneyCompact(progress.revenue)}</small></span></div><div className="sales-target-stats"><label><em>目标</em><b title={yuan(progress.target)}>{formatMoneyCompact(progress.target)}</b></label><label><em>预计月末</em><b title={yuan(progress.projectedRevenue)}>{formatMoneyCompact(progress.projectedRevenue)}</b></label><label><em>{progress.remaining < 0 ? "超出" : "还需"}</em><b title={yuan(Math.abs(progress.remaining))}>{formatMoneyCompact(Math.abs(progress.remaining))}</b></label></div></div><div className="sales-target-bullet" aria-label={`当前完成 ${progress.completionRate}%，预计月末 ${progress.projectedRate}%`}><i style={{ width: `${Math.min(100, progress.completionRate)}%` }} /><em style={{ left: `${projectedRate}%` }} /><b /><span><small>当前 {progress.completionRate}%</small><small>预计 {progress.projectedRate}%</small></span></div>{progress.completionRate > 120 && <div className="sales-target-review" role="status"><span><CircleAlert size={16} strokeWidth={1.5} aria-hidden="true" /><span><em>完成率 {progress.completionRate}%</em><b>目标可能偏低，建议调整下月目标</b></span></span><button type="button" onClick={onEdit}>复核目标 <Pencil size={16} strokeWidth={1.5} /></button></div>}<div className="target-foot"><p>{progress.revenue > 0 ? `${stateCopy}。` : "目标已记录；录入本月销售后，完成环会自动填充。"}</p><button onClick={onOpenOrders}>查看订单明细 <ChevronRight size={16} strokeWidth={1.5} /></button></div></section>;
}
...
export default function Home() {
  ...
  const profileIndustryInputRef = useRef<HTMLInputElement>(null);
  ...
  function OrdersPage() {
    ...
    return <div className="prototype-orders ledger-page-shell"><section className="orders-page-title ledger-page-heading"><h1>订单</h1><div><button onClick={() => setOrderSearchOpen((value) => !value)} aria-label="搜索订单"><Search size={20} strokeWidth={1.5} /></button><button onClick={openNewOrder} aria-label="新增订单"><Plus size={20} strokeWidth={1.5} /></button></div></section><div className="orders-month-row"><label className="month-filter"><CalendarDays size={18} strokeWidth={1.5} /><select aria-label="筛选订单月份" value={orderMonth} onChange={(event) => setOrderMonth(event.target.value)}><option value="all">全部月份</option>{orderMonths.map((month) => <option key={month} value={month}>{monthLabel(month)}</option>)}</select></label></div>{orderSearchOpen && <label className="search-field order-search"><Search size={16} strokeWidth={1.5} /><input autoFocus value={orderSearch} onChange={(event) => setOrderSearch(event.target.value)} placeholder="搜索订单号、客户或 SKU" /></label>}<div className="order-filter-chips"><button className={orderStatusFilter === "all" ? "active" : ""} onClick={() => setOrderStatusFilter("all")}>全部</button><button className={orderStatusFilter === "unreviewed" ? "active" : ""} onClick={() => setOrderStatusFilter("unreviewed")}>待复核</button><button className={orderStatusFilter === "low_profit" ? "active" : ""} onClick={() => setOrderStatusFilter("low_profit")}>低利润</button><button className={orderStatusFilter === "refund" ? "active" : ""} onClick={() => setOrderStatusFilter("refund")}>退款</button></div>{filteredOrders.length > 0 && <section className="order-batch-toolbar" aria-label="订单批量操作"><label><input type="checkbox" checked={allVisibleSelected} onChange={(event) => toggleAllVisible(event.target.checked)} aria-label="全选当前筛选订单" /><span>全选</span></label><strong>{selectedOrders.length ? `已选 ${selectedOrders.length} 笔` : "选择订单后批量处理"}</strong><div><button type="button" disabled={!selectedOrders.length} onClick={() => updateSelectedReview(true)}>标记复核</button><button type="button" disabled={!selectedOrders.length} onClick={() => updateSelectedReview(false)}>取消复核</button><button type="button" disabled={!selectedOrders.length} onClick={() => void exportSelectedOrders()}><Download size={16} strokeWidth={1.5} />导出</button></div></section>}<div className="order-list ledger-row-list">{filteredOrders.map((order) => { const afterSales = getOrderAfterSalesMetrics(order, refunds); const warning = orderWarnings.find((item) => item.orderId === order.id); const selected = selectedOrderIds.includes(order.id); return <article key={order.id} className={`order-list-item${selected ? " selected" : ""}${order.reviewedAt ? " reviewed" : ""}`}><label className="order-select"><input type="checkbox" checked={selected} onChange={(event) => toggleOrder(order.id, event.target.checked)} aria-label={`选择订单 ${order.orderNo || "未编号订单"}`} /><span aria-hidden="true" /></label><button className="order-list-main" onClick={() => openOrder(order.id)}><div><b><Highlight value={order.orderNo || "未编号订单"} query={orderSearch} /></b><em><Highlight value={order.buyer || "散客"} query={orderSearch} /></em><small><Highlight value={channelLabel[order.channel]} query={orderSearch} /> · {afterSales.hasAfterSale ? "有售后" : "无退款"} · <i className={order.reviewedAt ? "reviewed" : "unreviewed"}>{order.reviewedAt ? "已复核" : "待复核"}</i></small></div><span className="order-time">{order.occurredAt.slice(5)}<ChevronRight size={18} strokeWidth={1.5} /></span><strong className="order-after-sales-metric"><b>{yuan(fromFen(afterSales.netRevenueFen))}</b><em>退款后净收入</em><small className={warning ? "attention" : ""}>{warning ? (warning.type === "below_break_even" ? "低于保本" : "低于目标") : `成本 ${yuan(fromFen(afterSales.operatingCostFen))} · 贡献 ${yuan(fromFen(afterSales.operatingContributionFen))}`}</small></strong></button></article>; })}{!filteredOrders.length && <div className="empty-state business-empty detail-empty-state" data-testid="orders-empty">{hasFilter ? <><b>没有匹配结果</b><p>{`“${orderSearch || monthLabel(orderMonth)}”下暂无订单，试试更换月份或关键词。`}</p></> : <><span>＋</span><b>本期尚无商品销售订单</b><p>商品销售订单会联动收入、SKU 销量与成本快照；录入第一笔订单后即可逐笔核对净营收、已售成本与贡献利润。</p><button type="button" className="empty-action" onClick={openNewOrder}>记录订单</button>{!skus.length && <button type="button" className="orders-empty-secondary" onClick={openNewCard}>先建商品成本卡</button>}</>}</div>}</div><section className="after-sale-status ledger-surface"><div><b>售后处理</b><span>{periodRefunds.length ? "全部售后" : "本期无退款"}</span></div>{!periodRefunds.length && <p aria-hidden="true" />}{periodRefunds.length > 0 && <ChevronRight size={18} strokeWidth={1.5} />}</section>{refundPareto.length >= 2 && <section className="order-refund-chart ledger-surface"><div className="section-title"><div><span>售后原因</span><h2>退款占比</h2></div></div><RefundPareto items={refundPareto} onSelect={(reason) => { setOrderStatusFilter("refund"); setOrderSearch(reason); setOrderSearchOpen(true); }} /></section>}<button className="fixed-primary list-primary" onClick={openNewOrder}><Plus size={16} strokeWidth={1.5} />记录订单</button></div>;
  }

  function OrderFormPage() {
    if (!skus.length) return <><section className="sub-intro compact"><span>{template.label} · 订单入账</span><h1>记录订单</h1><p>请先建商品成本卡。订单需要关联 SKU，才会冻结成交时的直接已售成本。</p></section><section className="order-prerequisite"><span>记录订单前的准备</span><b>请先建商品成本卡</b><em>新建商品成本卡并填写售价后，系统会自动创建可下单 SKU；随后返回此页记录成交、佣金与履约费用。</em><button onClick={openNewCard}><Plus size={16} strokeWidth={1.5} />新建商品成本卡</button></section></>;
    const channelPricing = channelTemplates[orderChannel];
    return <><section className="sub-intro compact"><span>{template.label} · 订单入账</span><h1>记录订单</h1><p>订单会冻结当前渠道佣金、履约费用和目标贡献毛利率，后续修改模板不会重写历史订单。</p></section><form key="order-entry-form" className="record-form" onSubmit={saveOrder}><label>订单号（可选）<input name="orderNo" placeholder="例如：PDD-20260714-001" /></label><label><span className="sdq-label">销售渠道<i className="sdq-req" aria-hidden="true" /></span><select value={orderChannel} onChange={(event) => setOrderChannel(event.target.value as OrderChannel)}><option value="platform">平台店</option><option value="live">直播</option><option value="store">到店</option><option value="private">私域</option><option value="other">其他</option></select></label><section className="channel-template-preview"><span><Calculator size={16} strokeWidth={1.5} />{channelLabel[orderChannel]}默认费用模板</span><b>佣金 {channelPricing.commissionRatePct}% · 履约 ¥{channelPricing.fulfillmentCost} / 件 · 目标贡献毛利 {channelPricing.targetContributionMarginPct}%</b><em>可在成本卡的智能定价页修改并保存此渠道模板。</em></section><label>客户 / 收件人<input name="buyer" placeholder="例如：张女士 / 散客" /></label><label><span className="sdq-label">成交日期<i className="sdq-req" aria-hidden="true" /></span><input name="date" type="date" defaultValue={editingIndirectPool?.occurredAt || today} /></label><section className="detail-breakdown"><h2>SKU 明细</h2><div className="bom-list">{draftOrderLines.map((line, index) => { const sku = skus.find((item) => item.id === line.skuId); return <div key={`${line.skuId}-${index}`}><span><select value={line.skuId} onChange={(event) => setDraftOrderLines((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, skuId: event.target.value } : item))}>{skus.map((item) => <option value={item.id} key={item.id}>{item.code} · {item.name}</option>)}</select><em>{sku ? `${yuan(sku.unitPriceFen / 100)} / ${sku.unit} · 成本 ${yuan(sku.unitCostFen / 100)}` : "请选择 SKU"}</em></span><input aria-label="数量" type="number" min="1" step="1" value={line.quantity ?? ""} onChange={(event) => setDraftOrderLines((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, quantity: toEditableNumber(event.target.value) } : item))} placeholder="数量" /><button type="button" onClick={() => setDraftOrderLines((current) => current.filter((_, itemIndex) => itemIndex !== index))} aria-label="删除 SKU 明细"><Trash2 size={16} strokeWidth={1.5} /></button></div>; })}</div><button type="button" onClick={() => setDraftOrderLines((current) => [...current, { skuId: skus[0]?.id || "", quantity: 1 }])}><Plus size={16} strokeWidth={1.5} />添加 SKU</button></section><button type="submit" className="fixed-primary form-save"><Plus size={16} strokeWidth={1.5} />确认订单并入账</button></form></>;
  }
  ...
  function RefundFormPage() {
    ...
    return <><section className="sub-intro compact"><span>{activeOrder.orderNo} · {activeOrderLine.skuName}</span><h1>登记退款</h1><p>退款冲减净营收；仅可售回收入库会冲回已售成本，破损报废不冲回。</p></section><form className="record-form" onSubmit={saveRefund}><label><span className="sdq-label">退款数量<i className="sdq-req" aria-hidden="true" /></span><input name="quantity" type="number" min="1" max={available} step="1" defaultValue={available} /></label><label><span className="sdq-label">退款金额<i className="sdq-req" aria-hidden="true" /></span><div className="amount-input"><span>¥</span><input name="refundAmount" type="number" min="0.01" max={(activeOrderLine.unitPriceFen / 100 * available).toFixed(2)} step="0.01" defaultValue={(activeOrderLine.unitPriceFen / 100 * available).toFixed(2)} /></div></label><label>退款手续费（可选）<div className="amount-input"><span>¥</span><input name="refundFee" type="number" min="0" step="0.01" defaultValue="0" /></div></label><label><span className="sdq-label">退款原因<i className="sdq-req" aria-hidden="true" /></span><select name="reason"><option value="quality_issue">质量问题</option><option value="wrong_item">错发漏发</option><option value="customer_cancelled">客户取消</option><option value="logistics_delay">物流延误</option><option value="duplicate_order">重复下单</option><option value="other">其他</option></select></label><label><span className="sdq-label">退货回收状态<i className="sdq-req" aria-hidden="true" /></span><select name="recoveryStatus"><option value="not_returned">无需退货</option><option value="in_transit">退货在途</option><option value="sellable_restocked">可售回收入库（冲回成本）</option><option value="damaged_disposed">破损报废（不冲回成本）</option></select></label><label><span className="sdq-label">退款日期<i className="sdq-req" aria-hidden="true" /></span><input name="date" type="date" min={activeOrder.occurredAt} defaultValue={today < activeOrder.occurredAt ? activeOrder.occurredAt : today} /></label><button type="submit" className="fixed-primary form-save"><ReceiptText size={16} strokeWidth={1.5} />确认退款</button></form></>;
  }

  function SkusPage() {
    return <><section className="sub-intro compact"><span>{template.label} · {cardCopy.title}</span><h1>SKU {cardCopy.title}</h1><p>单位成本来自成本卡；订单销售、退款与可售回收会形成每个 SKU 的真实毛利。调整成本只影响之后创建的订单。</p></section><div className="cost-card-list">{skuMetrics.map((sku) => <div key={sku.id}><span className="healthy"><PackageOpen size={18} strokeWidth={1.5} /></span><div><b>{sku.name}</b><em>{sku.code} · 售 {sku.soldQuantity}{sku.unit} · 退 {sku.refundedQuantity}{sku.unit}</em><em>净营收 {yuan(sku.netRevenue)} · 已售成本 {yuan(sku.cogs)}</em><form onSubmit={(event) => saveSkuCost(event, sku.id)} className="sku-cost-form"><label><span className="sdq-label">单位成本 ¥<i className="sdq-req" aria-hidden="true" /></span><input name="unitCost" type="number" min="0" step="0.01" defaultValue={(sku.unitCostFen / 100).toFixed(2)} /></label><button type="submit">更新</button></form></div><strong>{sku.grossMarginRate}%<small>真实毛利</small></strong></div>)}{!skuMetrics.length && <div className="empty-state">当前行业没有 SKU。请先维护成本卡。</div>}</div></>;
  }
  ...
  function RecordsPage() {
    ...
    return <>
      ...
      <label className="search-field"><Search size={16} strokeWidth={1.5} /><input value={recordSearch} onChange={(event) => setRecordSearch(event.target.value)} placeholder="搜索商户、备注或分类" /></label>
      ...
      {filteredRecords.length > 0 && <section className="record-export-panel" aria-label="导出当前筛选账单">
        ...
        <div className="record-export-actions"><button type="button" onClick={() => exportFilteredRecords("csv")} disabled={!filteredRecords.length}><Download size={16} strokeWidth={1.5} />CSV</button><button type="button" onClick={() => exportFilteredRecords("xlsx")} disabled={!filteredRecords.length}><Download size={16} strokeWidth={1.5} />Excel</button></div>
      </section>}
      <section className="record-list">{groupedRecords.length === 0 && <div className="empty-state record-empty-state">{hasFilter ? <><b>没有匹配的流水</b><p>试试更换月份、类型或关键词。</p><button type="button" onClick={clearRecordFilters}>清除筛选</button></> : <><b>当前还没有流水</b><p>从一笔收入或成本开始记录。</p></>}</div>}{groupedRecords.map((group) => <div key={group.date} className="record-group" data-testid="record-group" data-date={group.date}><h3><span className="record-group-date">{book.dateLabel(group.date)}</span><span className="record-group-meta"><Highlight value={group.date.replaceAll("-", " / ")} query={recordSearch} /> · <b className="record-group-count" data-testid="record-group-count">{group.records.length} 笔</b></span></h3>{group.records.map((record) => { const category = categoryByKey.get(record.categoryKey); const isIncome = record.type === "income"; return <button className="record-row" data-testid="record-row" key={record.id} onClick={() => openRecordDetail(record.id)}><span className="record-icon" style={{ color: category?.color, background: `${category?.color || "#087FF5"}18` }}><ReceiptText size={18} strokeWidth={1.5} /></span><span className="record-main"><b><Highlight value={record.merchant} query={recordSearch} /></b><em><Highlight value={category?.label || "未分类"} query={recordSearch} /> · <Highlight value={record.note || "无备注"} query={recordSearch} />{record.hasAttachment ? " · 有凭证" : ""}</em></span><strong className={`record-amount${isIncome ? " income" : ""}`} data-testid="record-amount">{isIncome ? "+" : "−"}{yuan(record.amount)}</strong><ChevronRight size={18} strokeWidth={1.5} /></button>; })}</div>)}</section>
      <button className="floating-add" onClick={openNewRecord} aria-label="新增记一笔"><Plus size={20} strokeWidth={1.5} aria-hidden="true" />记一笔</button>
    </>;
  }
  ...
  function AnalysisPageContent() {
    ...
    return <div className="prototype-analysis ledger-page-shell analysis-diagnosis-page analysis-profit-page" data-chart-role={chartRole("analysis").primary}>
      ...
      <section className="analysis-structure-compare ledger-surface" data-chart-template="grouped-bars"><div className="analysis-card-head"><div><h2>成本结构变化</h2><span>正向成本占比</span></div><div className="analysis-grouped-legend" aria-label="图例"><span><i className="current" />本期</span><span><i className="previous" />上期</span></div></div><StructureComparisonGroupedBars key={period} items={structureComparison} onSelect={openStructureRecords} /></section>
      <section className="analysis-supplier-rank ledger-surface" data-chart-template="rank"><div className="analysis-card-head"><h2>成本归属 · 供应商</h2><span>已关联支出</span></div>{supplierRankings.length ? <div className="analysis-supplier-list template-supplier-lollipop">{supplierRankings.map((item, index) => <button key={item.supplierId} onClick={() => openSupplierRecords(item.supplierId)}><i>{index + 1}</i><span><b>{item.label}</b><em>{item.entryCount} 笔已入账支出 · 占已关联成本 {item.share}%</em><i className="supplier-lollipop" aria-hidden="true"><em style={{ width: `${item.amount / supplierRankMax * 100}%` }} /></i></span><strong>{yuan(item.amount)}</strong><ChevronRight size={18} strokeWidth={1.5} /></button>)}</div> : <button className="analysis-supplier-empty" onClick={() => { setRecordType("expense"); openNewRecord(); }}><span><b>尚未关联供应商支出</b><em>在成本记录里关联供应商，才能核对成本归属。</em></span><ChevronRight size={16} strokeWidth={1.5} /></button>}</section>
      </div>}</section>
    </div>;
  ...
  function ProfilePage() {
    ...
    return <><section className="profile-identity-card profile-account-summary"><div className="profile-identity-top"><BrandAvatar assetId={meQuery.data?.avatarAssetId} preset={meQuery.data?.avatarPreset} alt="个人头像" size="large" /><div><span>当前账号</span><h1>{meQuery.data?.name || "店铺经营者"}</h1><p>{meQuery.data?.email || "已登录工作区"} · {template.label}经营者</p></div><button onClick={() => goSub("profileSettings")} aria-label="编辑个人与店铺资料"><Pencil size={16} strokeWidth={1.5} /></button></div><div className="profile-identity-meta"><span><Check size={16} strokeWidth={1.5} />个人已验证</span><i /><span><Store size={16} strokeWidth={1.5} />{currentWorkspace?.name || template.storeName}</span></div></section><section className="profile-group"><h2>店铺管理</h2><div className="profile-card"><button onClick={() => goSub("profileSettings")}><span><Settings2 size={18} strokeWidth={1.5} />个人与店铺资料</span><strong>编辑<ChevronRight size={18} strokeWidth={1.5} /></strong></button><button onClick={() => goSub("industry")}><span><Store size={18} strokeWidth={1.5} />经营行业</span><strong>{template.label}<ChevronRight size={18} strokeWidth={1.5} /></strong></button><button onClick={() => goSub("budget")}><span><WalletCards size={18} strokeWidth={1.5} />经营预算</span><strong>{yuan(totals.budget)}<ChevronRight size={18} strokeWidth={1.5} /></strong></button></div></section><section className="profile-group"><h2>设置</h2><div className="profile-card"><button onClick={() => goSub("appearance")}><span><Sparkles size={18} strokeWidth={1.5} />皮肤中心</span><strong>{visualSkinOptions.find((option) => option.id === book.visualSkin)?.label || "外观"}<ChevronRight size={18} strokeWidth={1.5} /></strong></button></div></section><section className="profile-group profile-account-group"><h2>账户</h2><div className="profile-session"><span><em>当前账号</em><b>{meQuery.data?.email || "已登录工作区"}</b></span><button onClick={handleLogout} disabled={logout.isPending}><LogOut size={16} strokeWidth={1.5} />{logout.isPending ? "正在退出…" : "退出登录"}</button></div></section></>;
  }
  ...
  function RecordDetailPage() {
    ...
    return <><section className="detail-hero"><span className="detail-dot" style={{ background: category?.color || "#1677FF" }} /><span>{activeRecord.type === "income" ? "收入记录" : activeRecord.type === "refund" ? "退款记录" : "支出记录"}</span><h1>{activeRecord.merchant}</h1><strong>{activeRecord.type === "income" ? "+" : "-"}{yuan(activeRecord.amount)}</strong><p>{activeRecord.date} · {category?.label || "未分类"} · {activeRecord.status === "accounted" ? "已核算" : activeRecord.status === "pending" ? "待核算" : "异常"}</p></section><section className="detail-breakdown"><h2>记录说明</h2><div><span className="tip-icon"><ReceiptText size={18} strokeWidth={1.5} /></span><p>{activeRecord.note || "未填写备注"}{hasImageVoucher ? " · 已附图片凭证" : activeRecord.hasAttachment ? " · 已附凭证" : " · 未附凭证"}</p></div>{hasImageVoucher && <a className="record-voucher-preview" href={`/api/media/${activeRecord.attachmentAssetId}`} target="_blank" rel="noreferrer"><img src={`/api/media/${activeRecord.attachmentAssetId}`} alt={`${activeRecord.merchant}的凭证图片`} /><span>查看受保护凭证图片 <ChevronRight size={16} strokeWidth={1.5} /></span></a>}<button onClick={editRecord}>编辑记录 <Pencil size={16} strokeWidth={1.5} /></button><button className="danger-button" onClick={deleteRecord}>删除记录 <Trash2 size={16} strokeWidth={1.5} /></button></section></>;
  }
  ...
  function CardsPage() {
    ...
    return <div className="prototype-products ledger-page-shell"><section className="products-page-title ledger-page-heading"><h1>{cardCopy.title}</h1></section><div className="product-tabs"><button className="active">全部{template.entityLabel} <span>{cards.length}</span></button><button>关注 <span>{watchCount}</span></button><button><ChevronDown size={16} strokeWidth={1.5} />按成本上升</button></div><label className="search-field product-search"><Search size={16} strokeWidth={1.5} /><input value={cardSearch} onChange={(event) => setCardSearch(event.target.value)} placeholder={`搜索${template.entityLabel}名称或类型`} /></label><div className="product-cost-list ledger-row-list">{visibleCards.map((card) => { const allocatedIndirectCost = (indirectCostUnitFenByCard[card.id] || 0) / 100; const directValue = calcCard(card); const value = { ...directValue, cost: Number((directValue.cost + allocatedIndirectCost).toFixed(2)) }; const unitProfit = card.salePrice - value.cost; return <button key={card.id} onClick={() => openCard(card.id)}><div className="product-card-head"><CostCardThumbnail assetId={card.imageAssetId} alt={card.name} /><div><b>{card.name}</b><em>{card.kind || template.entityLabel} · 单位：{card.unit}</em></div><i className={card.status === "risk" ? "risk" : card.status === "attention" ? "attention" : "healthy"}>{card.status === "risk" ? "需复核" : card.status === "attention" ? "成本上升" : "稳定"}</i></div><div className="product-kpis ledger-data-columns"><label><em>单位成本</em><b>{yuan(value.cost)}</b></label><label><em>售价</em><b>{card.salePrice > 0 ? yuan(card.salePrice) : "未填写"}</b></label><label><em>单件利润</em><b className={unitProfit < 0 ? "negative" : "positive"}>{card.salePrice > 0 ? yuan(unitProfit) : "—"}</b></label></div><div className="product-actions"><span><Boxes size={16} strokeWidth={1.5} />成本构成 <b>{card.items.length}项</b><ChevronRight size={16} strokeWidth={1.5} /></span><span><Calculator size={16} strokeWidth={1.5} />智能定价 <ChevronRight size={16} strokeWidth={1.5} /></span></div></button>; })}{!visibleCards.length && <div className="empty-state">没有匹配的成本卡。</div>}</div><button className="fixed-primary list-primary" onClick={openNewCard}><Plus size={16} strokeWidth={1.5} />新增{template.entityLabel}成本卡</button></div>;
  }
  ...
  function CardDetailPage() {
    ...
    const costHistory = activeCard.history;
    const latestCost = costHistory.at(-1) ?? cardCost.cost;
    const previousCost = costHistory.length >= 2 ? costHistory.at(-2)! : null;
    const costDelta = previousCost === null ? null : Number((latestCost - previousCost).toFixed(2));
    const costDeltaRate = previousCost ? Number((costDelta! / previousCost * 100).toFixed(1)) : null;
    ...
    return <>
      ...
      {activeCard.salePrice > 0 ? <EquationResult firstLabel="当前售价" firstValue={yuan(activeCard.salePrice)} secondLabel="单位成本" secondValue={yuan(cardCost.cost)} resultLabel={cardCopy.unitProfitLabel} resultValue={yuan(unitProfit)} detail={`毛利率 ${cardCost.marginRate}% · ${unitProfit >= 0 ? "当前售价可覆盖单位成本" : "当前售价低于单位成本，请优先调整"}`} /> : <button className="pricing-empty-tip" onClick={openPricing}><Calculator size={18} strokeWidth={1.5} /><span><b>尚未填写售价，毛利率无法计算</b><em>使用智能测算定价，先设定目标毛利再一键写入。</em></span><ChevronRight size={16} strokeWidth={1.5} /></button>}
      <section className="detail-actions"><button className="primary" onClick={editCard}><Pencil size={16} strokeWidth={1.5} />编辑成本</button><button onClick={openPricing}><Calculator size={16} strokeWidth={1.5} />测算定价</button><button type="button" className="more-trigger" aria-expanded={cardMoreOpen} aria-controls="card-more-actions" onClick={() => setCardMoreOpen((open) => !open)}><MoreHorizontal size={16} strokeWidth={1.5} />更多</button>{cardMoreOpen && <div className="detail-more-menu" id="card-more-actions"><button type="button" className="danger" onClick={requestDeleteCard}><Trash2 size={16} strokeWidth={1.5} />删除成本卡</button></div>}</section>
      {linkedSku && <section className="detail-breakdown"><h2>订单 SKU 实绩</h2><div><span className="tip-icon"><ShoppingCart size={18} strokeWidth={1.5} /></span><p>售出 {linkedSku.soldQuantity}{linkedSku.unit}，退款 {linkedSku.refundedQuantity}{linkedSku.unit}，净营收 {yuan(linkedSku.netRevenue)}，真实毛利率 {linkedSku.grossMarginRate}% 。</p></div><button onClick={() => goSub("skus")}>查看 SKU 经营 <ChevronRight size={16} strokeWidth={1.5} /></button></section>}
      <section className="detail-breakdown"><h2>{template.formulaLabel}明细</h2><div className="bom-list">{activeCard.items.map((item) => <div key={item.id}><span><b>{item.name}</b><em>{item.spec || "规格待补充"} · {item.quantity}</em></span><strong>{yuan(item.amount)}</strong><span className="bom-row-actions"><button onClick={() => { setBomItemId(item.id); goSub("bomForm"); }} aria-label="编辑成本项"><Pencil size={16} strokeWidth={1.5} /></button><button onClick={() => { book.removeBomItem(activeCard.id, item.id); notify("已删除成本项，单位成本和 SKU 已同步重算"); }} aria-label="删除成本项"><Trash2 size={16} strokeWidth={1.5} /></button></span></div>)}</div><button onClick={() => { setBomItemId(null); goSub("bomForm"); }}>＋ 添加{template.formulaLabel}项</button></section>
      <section className="detail-breakdown"><h2>近 6 月单位成本趋势</h2><div className="weekly-bars cost-history">{activeCard.history.map((amount, index) => <span key={`${amount}-${index}`}><i style={{ height: `${amount / max * 100}%`, background: index === activeCard.history.length - 1 ? "#087FF5" : "#cfe2ff" }} /><em>{index + 2} 月</em></span>)}</div>{costDelta !== null && <div className="cost-history-delta" data-testid="cost-history-delta"><span>较上月</span><b className={costDelta >= 0 ? "up" : "down"}>{costDelta >= 0 ? "+" : "−"}{yuan(Math.abs(costDelta))} · {Math.abs(costDeltaRate!)}%</b></div>}</section>
    </>;
  }
  ...
  function PricingPage() {
    ...
    return <>
      ...
      <section className="record-form compact-form pricing-inputs">
        <label><span className="sdq-label">销售渠道<i className="sdq-req" aria-hidden="true" /></span><select value={pricingChannel} onChange={(event) => switchPricingChannel(event.target.value as OrderChannel)}>{(Object.keys(channelTemplates) as OrderChannel[]).map((channel) => <option key={channel} value={channel}>{channelLabel[channel]}</option>)}</select></label>
        <div className="template-head"><span><Calculator size={16} strokeWidth={1.5} />{channelLabel[pricingChannel]}费用模板</span><button type="button" onClick={() => { if (!pricingInputsReady) return notify("请补齐渠道费率、履约费用和目标毛利率"); const result = book.updateChannelPricing(pricingChannel, { commissionRatePct: base.platformRatePct, fulfillmentCost: base.fulfillmentCost, targetContributionMarginPct: targetMargin }); notify(result.ok ? `${channelLabel[pricingChannel]}默认费用模板已保存，新订单将自动使用` : result.reason); }}>保存模板</button></div>
        <label><span className="sdq-label">渠道综合费率<i className="sdq-req" aria-hidden="true" /></span><input type="number" min="0" max="99" step="0.1" value={pricingPlatformRate} onChange={(event) => { setPricingPlatformRate(toEditableNumber(event.target.value)); setPricingPreviewPrice(null); }} placeholder="例如：5" /><small>按订单实收比例扣除；非平台渠道可填 0%</small></label>
        <label><span className="sdq-label">单件履约费用<i className="sdq-req" aria-hidden="true" /></span><div className="amount-input"><span>¥</span><input type="number" min="0" step="0.01" value={pricingFulfillmentCost} onChange={(event) => { setPricingFulfillmentCost(toEditableNumber(event.target.value)); setPricingPreviewPrice(null); }} placeholder="例如：3.50" /></div><small>例如额外运费、支付手续费或单件售后准备金</small></label>
        <label><span className="sdq-label">目标贡献毛利率<i className="sdq-req" aria-hidden="true" /></span><input type="number" min="0" max="99" step="1" value={pricingTargetMargin} onChange={(event) => { setPricingTargetMargin(toEditableNumber(event.target.value)); setPricingPreviewPrice(null); }} placeholder="例如：40" /></label>
      </section>
      ...
      {suggested?.available && preview ? <section className="pricing-recommend" data-tone="navy"><span>{pricingPreviewPrice === null ? `建议售价 · 目标贡献毛利 ${targetMargin}%` : "当前试算价"}</span><strong>{yuan(preview.price)}</strong><p>{pricingPreviewPrice === null ? `直接保留两位小数；每${activeCard.unit}贡献 ${yuan(preview.contribution)}。` : `参考建议价 ${yuan(suggested.price)}；拖动后实时重算。`}</p><div className="price-slider-panel"><div><b>拖动试算售价</b><em>{priceState.copy}</em></div><input type="range" min={sliderMin} max={sliderMax} step="0.01" value={preview.price} aria-label="拖动试算售价" aria-valuetext={yuan(preview.price)} onChange={(event) => setPricingPreviewPrice(Number(Number(event.target.value).toFixed(2)))} /><div className="price-slider-scale"><span>保本 {yuan(breakEven?.price || 0)}</span><button type="button" onClick={() => setPricingPreviewPrice(null)}>回到建议价 {yuan(suggested.price)}</button><span>{yuan(sliderMax)}</span></div></div><div className={`pricing-preview-result ${priceState.tone}`}><b>贡献毛利 {preview.margin}%</b><em>每{activeCard.unit}贡献 {yuan(preview.contribution)} · {priceState.copy}</em></div><button onClick={() => writePrice(preview.price)}><Calculator size={16} strokeWidth={1.5} />写入当前试算价</button></section> : <div className="pricing-error"><CircleAlert size={18} strokeWidth={1.5} />{pricingInputsReady ? suggested?.reason : "请补齐渠道费率、履约费用和目标毛利率"}</div>}
      {profitTrend && <PricingProfitTrendChart trend={profitTrend} breakEvenPrice={breakEven?.price || 0} suggestedPrice={suggested?.price || 0} />}
      ...
      <section className="pricing-note"><CircleAlert size={16} strokeWidth={1.5} /><p>该测算是定价辅助工具，不替代市场需求、促销折扣、税费或实际平台结算核对；价格写入仅影响后续订单，历史订单售价和成本快照不会改写。</p></section>
    </>;
  }
  ...
  function CardFormPage() {
    ...
    return <><section className="sub-intro compact"><span>{template.label} · {template.entityLabel}定价</span><h1>{editing ? `编辑${template.entityLabel}` : `新增${template.entityLabel}`}</h1><p>可连续添加多项材料。保存后会自动创建或同步 SKU；更新只应用于之后创建的订单，历史订单保留成交快照。</p></section><form className="record-form secondary-form" onSubmit={saveCard} onChange={() => setCardFormDirty(true)}><label><span className="sdq-label">{template.entityLabel}名称<i className="sdq-req" aria-hidden="true" /></span><input name="name" defaultValue={editing?.name || ""} placeholder={`例如：${template.entityLabel}名称`} autoFocus /></label><label>类型 / 标签<input name="kind" defaultValue={editing?.kind || ""} placeholder={`例如：${template.entityLabel}类型`} /></label><label><span className="sdq-label">计量单位<i className="sdq-req" aria-hidden="true" /></span><input name="unit" defaultValue={editing?.unit || template.unitLabel} placeholder={`例如：${template.unitLabel}`} /></label><label>销售单价<div className="amount-input"><span>¥</span><input name="salePrice" type="number" min="0.01" step="0.01" defaultValue={editing?.salePrice || ""} placeholder="例如：68" /></div></label><section className="form-section material-editor"><span>基础材料清单</span><div className="material-list">{visibleMaterials.map((material, index) => <div className="material-row" key={material.id}><div className="material-row-head"><b>材料 {index + 1}</b>{visibleMaterials.length > 1 && <button type="button" onClick={() => setDraftMaterials((current) => removeDraftMaterial(current, material.id))}><Trash2 size={16} strokeWidth={1.5} />删除</button>}</div><div className="form-two-col"><label>名称<input value={material.name} onChange={(event) => updateMaterial(material.id, { name: event.target.value })} placeholder="例如：包装盒" /></label><label>金额<input type="number" min="0.01" step="0.01" value={material.amount} onChange={(event) => updateMaterial(material.id, { amount: toEditableNumber(event.target.value) })} placeholder="0.00" /></label><label>规格<input value={material.spec} onChange={(event) => updateMaterial(material.id, { spec: event.target.value })} placeholder="例如：500g" /></label><label>数量<input value={material.quantity} onChange={(event) => updateMaterial(material.id, { quantity: event.target.value })} placeholder={`例如：1 ${template.unitLabel}`} /></label></div></div>)}</div><button className="add-material" type="button" onClick={() => setDraftMaterials((current) => [...(current.length ? current : visibleMaterials), createDraftMaterial(template.unitLabel)])}><Plus size={16} strokeWidth={1.5} />新增材料</button></section><section className="form-section"><span>其他单位成本</span><div className="form-two-col"><label>人工分摊<input name="labor" type="number" min="0" step="0.01" defaultValue={editing?.labor ?? "0"} placeholder="0.00" /></label><label>固定分摊<input name="overhead" type="number" min="0" step="0.01" defaultValue={editing?.overhead ?? "0"} placeholder="0.00" /></label></div></section><div className="form-actions"><button className="form-secondary" type="button" onClick={goBack}>取消并返回</button><button className="fixed-primary form-save" type="submit"><Plus size={16} strokeWidth={1.5} />{editing ? "保存成本卡" : `创建${template.entityLabel}成本卡与 SKU`}</button></div></form></>;
  }

  function BomFormPage() {
    return <><section className="sub-intro compact"><span>{activeCard?.name || template.entityLabel} · {template.formulaLabel}</span><h1>{activeBomItem ? "编辑成本项" : "添加成本项"}</h1><p>保存后会立刻重算单位成本、毛利率和后续订单使用的 SKU 成本。</p></section><form className="record-form secondary-form" onSubmit={saveBomItem}><label><span className="sdq-label">成本项名称<i className="sdq-req" aria-hidden="true" /></span><input name="name" defaultValue={activeBomItem?.name || ""} placeholder="例如：包装盒 / 服务耗材" /></label><label>规格<input name="spec" defaultValue={activeBomItem?.spec || ""} placeholder="例如：500g / 单次" /></label><label><span className="sdq-label">数量<i className="sdq-req" aria-hidden="true" /></span><input name="quantity" defaultValue={activeBomItem?.quantity || "1 份"} placeholder="例如：1 份" /></label><label><span className="sdq-label">金额<i className="sdq-req" aria-hidden="true" /></span><div className="amount-input"><span>¥</span><input name="amount" type="number" min="0.01" step="0.01" defaultValue={activeBomItem?.amount ?? ""} placeholder="0.00" /></div></label><div className="form-actions"><button className="fixed-primary form-save" type="submit"><Plus size={16} strokeWidth={1.5} />{activeBomItem ? "保存并重算" : "加入并重算"}</button></div></form></>;
  }
  ...
  function ReportDetailPage() {
    ...
    return <><section className="detail-hero"><span>{template.label} · {activeReport.status === "closed" ? "已封存快照" : "实时草稿"}</span><h1>{activeReport.month.replace("-", " 年 ")} 月报</h1><strong>{yuan(activeReport.cost)}</strong><p>净营收 {yuan(activeReport.revenue)} · 毛利 {yuan(activeReport.margin)} · 毛利率 {activeReport.grossMarginRate}% · 经营利润率 {activeReport.operatingMarginRate}%</p></section><section className="detail-breakdown"><h2>订单与退款</h2><div><span className="tip-icon"><ShoppingCart size={18} strokeWidth={1.5} /></span><p>本期已纳入 {reportOrders.length} 笔订单、{reportRefunds.length} 笔 SKU 退款；订单销售和已售成本已并入收入与成本口径。</p></div><button onClick={() => goSub("orders")}>查看订单账本 <ChevronRight size={16} strokeWidth={1.5} /></button></section><section className="detail-breakdown"><h2>分类明细</h2><div className="report-breakdown">{activeReport.snapshot.map((item) => <span key={item.key}><b>{item.label}</b><em>{yuan(item.amount)} · {item.pct}%</em></span>)}</div>{activeReport.status === "draft" ? <button onClick={() => { book.closeReport(activeReport.month); notify("月报已封存；之后的流水不会改写该快照"); }}>封存本月口径 <FileText size={16} strokeWidth={1.5} /></button> : <button onClick={() => exportReportCsv(activeReport)}>导出 CSV <FileText size={16} strokeWidth={1.5} /></button>}</section></>;
  }
  ...
  function SuppliersPage() {
    ...
    return <>
      ...
      <label className="search-field supplier-search"><Search size={16} strokeWidth={1.5} /><input value={supplierSearch} onChange={(event) => setSupplierSearch(event.target.value)} placeholder="搜索供应商、联系人或分类" /></label>
      ...
      <button className="fixed-primary" onClick={() => { setSupplierEditId(null); goSub("supplierForm"); }}><Plus size={16} strokeWidth={1.5} />新增供应商</button>
    </>;
  }

  function SupplierFormPage() {
    const editing = suppliers.find((supplier) => supplier.id === supplierEditId);
    return <><section className="sub-intro compact"><span>{template.label} · 供应商信息</span><h1>{editing ? "编辑供应商" : "新增供应商"}</h1><p>共享供应商可在其他行业账本中选用；已有历史流水不会被改写。</p></section><form className="record-form secondary-form" onSubmit={saveSupplier}><label><span className="sdq-label">供应商名称<i className="sdq-req" aria-hidden="true" /></span><input name="name" defaultValue={editing?.name} placeholder="例如：优选食材供应" /></label><label>联系人<input name="contact" defaultValue={editing?.contact} placeholder="例如：李经理" /></label><label><span className="sdq-label">默认分类<i className="sdq-req" aria-hidden="true" /></span><select name="categoryKey" defaultValue={editing?.categoryKey || categories[0]?.key}>{categories.map((category) => <option value={category.key} key={category.key}>{category.label}</option>)}</select></label><label className="attachment-row"><span>跨行业可用</span><input name="shared" type="checkbox" defaultChecked={editing?.industryIds.includes("shared")} /></label><div className="form-actions"><button type="submit" className="fixed-primary form-save"><Plus size={16} strokeWidth={1.5} />{editing ? "保存供应商" : "新增供应商"}</button></div></form></>;
  }

  function CategoriesPage() {
    return <><section className="sub-intro compact"><span>{template.label} · 账本口径</span><h1>分类管理</h1><p>分类会同步影响记账归集、成本构成、报表和隐性成本模型。</p></section><div className="category-manage-list">{categories.map((category) => { const openEdit = () => { setCategoryEditId(category.id); goSub("categoryForm"); }; return <div key={category.id} className="manage-row" role="button" tabIndex={0} aria-label={`编辑分类 ${category.label}`} onClick={openEdit} onKeyDown={(event) => { if ((event.key === "Enter" || event.key === " ") && event.target === event.currentTarget) { event.preventDefault(); openEdit(); } }}><i style={{ background: category.color }} /><span><b>{category.label}</b><em>{category.hint}</em></span><ListRowMoreMenu id={`category-${category.id}`} editLabel="编辑分类" deleteLabel="删除分类" onEdit={openEdit} onDelete={() => deleteCategory(category.id, category.label)} /></div>; })}</div><button className="fixed-primary" onClick={() => { setCategoryEditId(null); goSub("categoryForm"); }}><Plus size={16} strokeWidth={1.5} />新增分类</button></>;
  }

  function CategoryFormPage() {
    const editing = categories.find((category) => category.id === categoryEditId);
    return <><section className="sub-intro compact"><span>{template.label} · 分类口径</span><h1>{editing ? "编辑分类" : "新增分类"}</h1><p>已占用分类不可删除；编辑显示名称、提示和颜色不会改写历史流水的分类键。</p></section><form className="record-form secondary-form" onSubmit={saveCategory}><label><span className="sdq-label">分类名称<i className="sdq-req" aria-hidden="true" /></span><input name="label" defaultValue={editing?.label} placeholder="例如：内容制作" /></label><label>分类提示<input name="hint" defaultValue={editing?.hint} placeholder="例如：活动素材、脚本与剪辑" /></label><label><span className="sdq-label">分类颜色<i className="sdq-req" aria-hidden="true" /></span><select name="color" defaultValue={editing?.color || "#1677FF"}><option value="#1677FF">品牌蓝</option><option value="#12B76A">绿色</option><option value="#F79009">橙色</option><option value="#7F56D9">紫色</option><option value="#F04438">红色</option></select></label><div className="form-actions"><button type="submit" className="fixed-primary form-save"><Plus size={16} strokeWidth={1.5} />{editing ? "保存分类" : "新增分类"}</button></div></form></>;
  }

  function NotificationsPage() {
    return <div className="notification-center"><section className="notification-summary"><span><Bell size={16} strokeWidth={1.5} />消息中心</span><b>{unreadNotificationCount ? `${unreadNotificationCount} 条待查看` : "已全部查看"}</b><button onClick={() => setReadNotificationIds(notificationItems.map((item) => item.id))}>{unreadNotificationCount ? "全部标为已读" : "全部已读"}</button></section><section className="notification-list">{notificationItems.map((item) => { const read = readNotificationIds.includes(item.id); return <button key={item.id} className={`${item.tone}${read ? " read" : ""}`} onClick={() => openNotificationTarget(item)}><span className="notification-symbol">{item.tone === "risk" ? "!" : item.tone === "attention" ? "·" : "＝"}</span><div><span className="notification-impact">{notificationImpact(item)}</span><b>{item.title}</b><em>{item.copy}</em><small>＝ {item.action}</small></div><ChevronRight size={18} strokeWidth={1.5} /></button>; })}</section></div>;
  }
  ...
  function ProfileSettingsPage() {
    ...
    if (!currentWorkspace || !me) return profileFailed
      ? <div className="empty-state profile-load-state" role="alert"><b>个人与店铺资料暂时无法读取</b><p>请检查网络或重新登录后再试；资料不会因此丢失。</p><div className="empty-state-actions"><button type="button" onClick={retryProfile}>重新加载</button><button type="button" onClick={goBack}>返回我的</button></div></div>
      : <div className="profile-skeleton" role="status" aria-label="正在加载个人与店铺资料"><span className="profile-skeleton-head"><i className="profile-skeleton-avatar" /><i className="profile-skeleton-row" /><i className="profile-skeleton-row" /></span><span className="profile-skeleton-card"><i className="profile-skeleton-row" /></span><span className="profile-skeleton-card"><i className="profile-skeleton-row" /></span></div>;
    ...
    return <form className="record-form profile-settings-form" onSubmit={save}><section className="sub-intro compact"><span>账户与店铺</span><h1>个人与店铺资料</h1><p>个人头像仅用于账户身份；店铺标识用于店铺展示；{cardCopy.imageManagementCopy}</p></section><section className="identity-settings-section"><div className="identity-settings-head"><BrandAvatar assetId={me.avatarAssetId} preset={me.avatarPreset} alt="个人头像" size="normal" /><span><b>个人头像</b><em>真实头像优先于预设；仅用于“我的”账户身份。</em></span></div><div className="avatar-preset-grid">{avatarPresets.map(({ id, label }) => <label key={id} className={(me.avatarPreset || "classic") === id ? "selected" : ""}><input type="radio" name="avatarPreset" value={id} defaultChecked={(me.avatarPreset || "classic") === id} /><BrandAvatar preset={id} alt={label} /><span>{label}</span></label>)}</div><label className="identity-inline-upload">上传真实头像<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event => upload(event.target.files?.[0], "user_avatar")} /></label></section><section className="identity-settings-section store-settings-section"><div className="identity-settings-head"><BrandStoreLogo assetId={currentWorkspace.logoAssetId} preset={currentWorkspace.logoPreset} alt="店铺品牌标识" size="normal" /><span><b>店铺品牌</b><em>私有 Logo 优先于预设；用于首页标题栏和店铺资料。</em></span></div><div className="avatar-preset-grid store-preset-grid">{storePresets.map(({ id, label }) => <label key={id} className={(currentWorkspace.logoPreset || "store") === id ? "selected" : ""}><input type="radio" name="logoPreset" value={id} defaultChecked={(currentWorkspace.logoPreset || "store") === id} /><BrandStoreLogo preset={id} alt={label} /><span>{label}</span></label>)}</div><label className="identity-inline-upload">上传私有店铺 Logo<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event => upload(event.target.files?.[0], "workspace_logo")} /></label></section><label><span className="sdq-label">您的姓名<i className="sdq-req" aria-hidden="true" /></span><input name="name" defaultValue={me.name} /></label><label><span className="sdq-label">店铺名称<i className="sdq-req" aria-hidden="true" /></span><input name="workspaceName" defaultValue={currentWorkspace.name} /></label><label><span className="sdq-label">经营行业<i className="sdq-req" aria-hidden="true" /></span><input type="hidden" name="industryId" ref={profileIndustryInputRef} defaultValue={currentWorkspace.industryId} /><Select defaultValue={currentWorkspace.industryId} onValueChange={value => { if (profileIndustryInputRef.current) profileIndustryInputRef.current.value = value; }}><SelectTrigger className="profile-industry-trigger" aria-label="经营行业"><SelectValue placeholder="请选择经营行业" /></SelectTrigger><SelectContent><SelectItem value="canteen">餐饮</SelectItem><SelectItem value="retail">零售</SelectItem><SelectItem value="ecommerce">电商</SelectItem><SelectItem value="beauty">美业服务</SelectItem><SelectItem value="stall">小商贩</SelectItem></SelectContent></Select></label><label>店铺联系人<input name="contactName" defaultValue={currentWorkspace.contactName || me.name || ""} /></label><button className="fixed-primary form-save" type="submit"><Check size={16} strokeWidth={1.5} />保存资料</button></form>;
  }
