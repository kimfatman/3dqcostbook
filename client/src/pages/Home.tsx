...
export default function Home() {
  ...
  const { template, categories, records, cards, suppliers, reports, totals, trend, hiddenCosts } = book;
  ...
  function saveRecord(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { categoryKey: currentCategoryKey, date, type: recordType, amount, merchant, note, status: "accounted" as const, hasAttachment };
    if (recordId) {
  ...
  function HomePage() {
    ...
    return <>
      <section className="dashboard-kicker"><span>工作台 · 2026 年 7 月</span><h1><IndustryIcon size={20} />{template.label}经营仪表盘</h1><em>{template.storeName} · {template.entityLabel} / {template.formulaLabel}</em></section>
      <section className="summary-card"><div className="summary-head"><span><IndustryIcon size={16} />{template.label} · 成本总览</span><button onClick={() => goSub("industry")}>行业模板 <ChevronRight size={14} /></button></div><div className="summary-main"><div><span>本月总成本</span><strong>{yuan(totals.totalCost)}</strong><em>较上月 <b>+{Math.max(0.1, (trend[5].cost / trend[4].cost - 1) * 100).toFixed(1)}%</b></em></div><img src="/manus-storage/mobile-cost-dashboard_efecf7a5.png" alt="成本数据插画" /></div><div className="summary-grid"><span><b>{totals.budgetUsed.toFixed(1)}%</b>预算已用</span><span><b>{totals.marginRate}%</b>毛利率</span><span><b>{yuan(Math.max(totals.budget - totals.totalCost, 0))}</b>可用余额</span></div><div className="summary-risk"><CircleAlert size={14} /><span>优先风险</span><strong>{template.risk}</strong></div></section>
      <button className="risk-card" onClick={() => setTab("analysis")}><span className="risk-symbol"><CircleAlert size={19} /></span><span><em>优先风险</em><strong>{template.risk}</strong><small>{template.riskNote}</small></span><ChevronRight size={18} /></button>
      ...
      <section className="mini-trend-card"><div><span>近 6 月经营趋势</span><strong>{trend[5].marginRate}% 毛利率</strong></div><div className="mini-bars">{trend.map((point, index) => <i key={point.month} style={{ height: `${Math.max(20, point.cost / Math.max(...trend.map((item) => item.revenue)) * 100)}%`, background: index === trend.length - 1 ? "#1677FF" : "#bddbff" }} />)}</div><button onClick={() => setTab("analysis")}><LineChart size={15} />查看趋势</button></section>
    </>;
  }
  ...
  function AnalysisPage() {
    const multiplier = analysisPeriod === "current" ? 1 : .945;
    const periodCost = totals.totalCost * multiplier;
    ...
    return <><section className="screen-title"><span>成本分析</span><h1>把花销变成判断</h1><p>保留原始构成、趋势、排行、供应商与隐性成本图表。</p></section><div className="segment-control"><button className={analysisPeriod === "current" ? "active" : ""} onClick={() => setAnalysisPeriod("current")}>本月</button><button className={analysisPeriod === "last" ? "active" : ""} onClick={() => setAnalysisPeriod("last")}>上月</button></div><section className="analysis-hero"><div><span>{analysisPeriod === "current" ? "本月" : "上月"}成本</span><strong>{yuan(periodCost)}</strong><em>预算占用 {totals.budgetUsed.toFixed(1)}%</em></div><div className="ring" style={{ background: `conic-gradient(${totals.categoryTotals.map((item, index) => `${item.color} ${totals.categoryTotals.slice(0, index).reduce((sum, value) => sum + value.amount, 0) / Math.max(totals.totalCost, 1) * 100}% ${totals.categoryTotals.slice(0, index + 1).reduce((sum, value) => sum + value.amount, 0) / Math.max(totals.totalCost, 1) * 100}%`).join(",")}` }}><i><b>{totals.marginRate}%</b><small>毛利率</small></i></div></section><section className="metric-strip"><div><span>收入</span><b>{yuan(totals.revenue * multiplier)}</b></div><div><span>毛利率</span><b>{totals.marginRate}%</b></div><div><span>趋势</span><b className="up"><TrendingUp size={14} />改善</b></div></section>
      ...
      <section className="section-block"><div className="section-title"><div><span>成本构成</span><h2>按金额排序</h2></div></div><div className="ranking-card">{totals.categoryTotals.map((item, index) => <button key={item.key} onClick={() => { setRecordSearch(item.label); setTab("records"); }}><span className="rank">{String(index + 1).padStart(2, "0")}</span><span className="rank-name"><i style={{ background: item.color }} />{item.label}</span><span className="rank-bar"><b style={{ width: `${item.amount / Math.max(totals.categoryTotals[0]?.amount || 1, 1) * 100}%`, background: item.color }} /></span><strong>{Math.round(item.amount / Math.max(totals.totalCost, 1) * 100)}%</strong></button>)}</div></section>
      ...
      <section className="hidden-cost-card"><div><span>行业隐性成本</span><h2>{template.label}漏损估算</h2><strong>{yuan(hiddenCosts.reduce((sum, item) => sum + item.estimate, 0))}</strong><em>约占营收 {(hiddenCosts.reduce((sum, item) => sum + item.estimate, 0) / Math.max(totals.revenue, 1) * 100).toFixed(1)}%</em></div><CircleAlert size={28} />{hiddenCosts.map((item) => <div className="hidden-row" key={item.key}><span>{item.label}</span><b>{yuan(item.estimate)}</b><i><em style={{ width: `${item.health}%` }} /></i></div>)}</section>
    </>;
  }
  ...
  function RecordPage() {
    ...
    return <><section className="sub-intro compact"><span>{template.label} · 成本记账</span><h1>{editing ? "编辑一笔记录" : "记录一笔收支"}</h1><p>保存后会真正写入当前行业流水，并同步预算、构成图和报告数据。</p></section><form className="record-form" onSubmit={saveRecord}><label>收支类型<div className="type-switch"><button type="button" className={recordType === "expense" ? "selected" : ""} onClick={() => setRecordType("expense")}>支出</button><button type="button" className={recordType === "income" ? "selected" : ""} onClick={() => setRecordType("income")}>收入</button><button type="button" className={recordType === "refund" ? "selected" : ""} onClick={() => setRecordType("refund")}>退款</button></div></label><label>金额<div className="amount-input"><span>¥</span><input name="amount" type="number" min="0.01" step="0.01" value={formAmount} onChange={(event) => setFormAmount(event.target.value)} placeholder="0.00" autoFocus /></div></label><label>日期<input name="date" type="date" value={formDate} onChange={(event) => setFormDate(event.target.value)} /></label><label>成本分类<div className="category-chips">{categories.map((item) => <button type="button" key={item.key} className={currentCategoryKey === item.key ? "selected" : ""} onClick={() => setSelectedCategoryKey(item.key)}>{item.label}</button>)}</div></label><label>商户 / 对方<input name="merchant" value={formMerchant} onChange={(event) => setFormMerchant(event.target.value)} placeholder="例如：平台服务商" /></label><label>备注<input name="note" value={formNote} onChange={(event) => setFormNote(event.target.value)} placeholder={`例如：${categories[0]?.hint || "本次支出"}`} /></label><label className="attachment-row"><span><Upload size={16} />凭证状态</span><button type="button" className={hasAttachment ? "attachment-on" : ""} onClick={() => setHasAttachment((value) => !value)}>{hasAttachment ? "已附凭证" : "添加凭证"}</button></label><button type="submit" className="fixed-primary form-save"><Plus size={18} />{editing ? "保存修改" : "保存记录"}</button></form></>;
  }
  ...
  function BudgetPage() {
    return <><section className="detail-hero budget-detail"><span>{template.label}月度预算</span><h1>{yuan(totals.budget)}</h1><strong>{totals.budgetUsed.toFixed(1)}% 已用</strong><p>已用 {yuan(totals.totalCost)}，本月仍可使用 {yuan(Math.max(totals.budget - totals.totalCost, 0))}。</p></section><form className="record-form compact-form" onSubmit={saveBudget}><label>调整本月成本预算<div className="amount-input"><span>¥</span><input name="budget" type="number" min="1" defaultValue={totals.budget} /></div></label><button className="fixed-primary form-save" type="submit">保存预算</button></form><section className="detail-breakdown"><h2>预算建议</h2><div><span className="tip-icon"><CircleAlert size={18} /></span><p>{template.riskNote}</p></div><button onClick={() => notify("已开启 80% 预算提醒")}>开启 80% 提醒 <ChevronRight size={16} /></button></section></>;
  }

  function ReportsPage() {
    return <><section className="sub-intro compact"><span>{template.label} · 月度经营</span><h1>成本报表</h1><p>每期报表锁定所属行业模板与分类口径，后续切换行业不改写历史。</p></section><div className="report-list">{reports.map((report) => <button key={report.id} onClick={() => openReport(report.id)}><span><FileText size={20} /></span><div><b>{report.month.replace("-", " 年 ")} 月成本报表</b><em>成本 {yuan(report.cost)} · 收入 {yuan(report.revenue)}</em></div><strong>{(report.margin / Math.max(report.revenue, 1) * 100).toFixed(1)}%<small>毛利率</small></strong><ChevronRight size={16} /></button>)}</div></>;
  }

  function ReportDetailPage() {
    if (!activeReport) return <div className="empty-state">报表不存在。</div>;
    return <><section className="detail-hero"><span>{template.label} · 模板口径已锁定</span><h1>{activeReport.month.replace("-", " 年 ")} 月报</h1><strong>{yuan(activeReport.cost)}</strong><p>收入 {yuan(activeReport.revenue)} · 毛利 {yuan(activeReport.margin)} · 毛利率 {(activeReport.margin / Math.max(activeReport.revenue, 1) * 100).toFixed(1)}%</p></section><section className="detail-breakdown"><h2>分类明细</h2><div className="report-breakdown">{activeReport.snapshot.map((item) => <span key={item.key}><b>{item.label}</b><em>{yuan(item.amount)} · {item.pct}% · 环比 {item.delta > 0 ? "+" : ""}{item.delta}pp</em></span>)}</div><button onClick={() => notify("报表已生成导出任务（演示）")}>导出报表 <FileText size={16} /></button></section></>;
  }
