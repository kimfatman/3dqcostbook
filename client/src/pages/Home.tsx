...
function ProfitWaterfall({ revenue, cogs, expenses, profit, onSelect }: { revenue: number; cogs: number; expenses: number; profit: number; onSelect: (key: "revenue" | "cogs" | "expenses" | "profit") => void }) {
  ...
  return <section className="profit-waterfall template-waterfall"><div className="chart-heading"><span>本期利润形成</span><b>{profit >= 0 ? "经营利润" : "经营亏损"} {yuan(Math.abs(profit))}</b><small>从成交收入到经营结果</small></div><div className="waterfall-plot">{steps.map((step) => { const bottom = (Math.min(step.from, step.to) - low) / range * 100; const height = Math.max(5, Math.abs(step.to - step.from) / range * 100); return <button key={step.key} className={step.kind} onClick={() => onSelect(step.key)}><i style={{ bottom: `${bottom}%`, height: `${height}%` }} /><strong>{step.amount < 0 ? "−" : ""}{yuan(Math.abs(step.amount))}</strong><span>{step.label}</span></button>; })}</div><div className="waterfall-insights" aria-label={`毛利率 ${grossMargin.toFixed(1)}%，费用率 ${expenseRate.toFixed(1)}%`}><span><em>毛利</em><b>{yuan(grossProfit)}</b></span><span><em>毛利率</em><b>{grossMargin.toFixed(1)}%</b></span><span><em>费用率</em><b>{expenseRate.toFixed(1)}%</b></span></div></section>;
}
...
function StructureComparisonGroupedBars({ items, onSelect }: { items: ReturnType<typeof buildCostStructureComparison>; onSelect: (key: string, label: string) => void }) {
  ...
  return <div className="analysis-grouped-comparison template-grouped-structure" aria-label="本期与上期正向成本占比分组对比">{items.map((item) => <button key={item.key} onClick={() => onSelect(item.key, item.label)}><span className="analysis-grouped-copy"><b>{item.label}</b><em>本期 {item.share}% · 上期 {item.previousShare}%</em></span><span className="analysis-grouped-bars" role="img" aria-label={`${item.label}：本期 ${item.share}%，上期 ${item.previousShare}%`}><i className="current" style={{ height: `${item.share / maxShare * 100}%` }} /><i className="previous" style={{ height: `${item.previousShare / maxShare * 100}%` }} /></span><strong className={item.shareDelta > 0 ? "up" : item.shareDelta < 0 ? "down" : "flat"}>{item.shareDelta > 0 ? "+" : ""}{item.shareDelta}pt</strong><ChevronRight size={15} /></button>)}</div>;
}
...
export default function Home() {
  ...
  function AnalysisPageContent() {
    ...
    const costSummary = buildCostAnalysisSummary(categoryDeltas) as Omit<ReturnType<typeof buildCostAnalysisSummary>, "deltaRate"> & { deltaRate: number };
    ...
    const openCategoryRecords = (key: string, label: string) => { setRecordSearch(label); setRecordMonth(period); goSub("records"); };
    return <div className="prototype-analysis ledger-page-shell" data-chart-role={chartRole("analysis").primary}>
      <section className="prototype-analysis-title ledger-page-heading"><h1>成本分析 · {period.replace("-", " 年 ")} 月</h1></section>
      <section className="analysis-cost-overview ledger-surface"><div className="analysis-card-head"><h2>本期成本结论</h2><div className="segment-control"><button className={analysisPeriod === "current" ? "active" : ""} onClick={() => setAnalysisPeriod("current")}>本月</button><button className={analysisPeriod === "last" ? "active" : ""} onClick={() => setAnalysisPeriod("last")}>上月</button></div></div>{costSummary.totalCost > 0 ? <div className="analysis-cost-total"><span>已入账净成本</span><strong>{yuan(costSummary.totalCost)}</strong><em className={costSummary.delta > 0 ? "up" : costSummary.delta < 0 ? "down" : "flat"}>{costSummary.deltaRate === null ? "暂无上期对比" : `${costSummary.delta >= 0 ? "较上期 +" : "较上期 "}${yuan(costSummary.delta)} · ${Math.abs(costSummary.deltaRate)}%`}</em>{costSummary.costCredits > 0 && <small className="analysis-cost-credit-note">已扣除成本回冲 {yuan(costSummary.costCredits)}；正向成本 {yuan(costSummary.grossCost)}</small>}</div> : <button className="analysis-no-revenue" onClick={openNewRecord}><span>本期尚未录入成本</span><b>补录成本后生成成本结论</b><ChevronRight size={16} /></button>}</section>
      <CostAnalysisPieChart items={costSummary.structure} grossCost={costSummary.grossCost} onSelect={openCategoryRecords} onOpenAll={() => { setRecordFilter("expense"); setRecordMonth(period); setRecordSearch(""); goSub("records"); }} />
      <section className="analysis-more ledger-surface"><button className="analysis-more-trigger" type="button" aria-expanded={analysisDetailsOpen} onClick={() => setAnalysisDetailsOpen((value) => !value)}><span><b>趋势与风险复核</b><em>查看利润、月度趋势和行业漏损估算</em></span><ChevronDown size={18} /></button>{analysisDetailsOpen && <div className="analysis-more-content"><section className="analysis-waterfall-card"><div className="analysis-card-head"><h2>利润与成本</h2></div>{hasRevenue ? <ProfitWaterfall revenue={analysisRevenue} cogs={analysisCogs} expenses={analysisExpenses} profit={periodView.totals.operatingProfit} onSelect={(key) => { if (key === "cogs" || key === "expenses") { setRecordFilter("expense"); setRecordMonth(period); goSub("records"); } else if (key === "revenue") { setRecordFilter("income"); setRecordMonth(period); goSub("records"); } else goSub("reports"); }} /> : <button className="analysis-no-revenue" onClick={openNewRecord}><span>本期尚未录入销售收入</span><b>补录收入后生成利润图</b><ChevronRight size={16} /></button>}</section><section className={`analysis-trend-card ${sparseTrend ? "sparse" : ""}`}><div className="analysis-card-head"><h2>近 6 月收入与成本</h2><div className="chart-legend"><span><i className="revenue" />收入</span><span><i className="cost" />成本</span></div></div><div className="trend-chart">{trend.map((point) => <button type="button" key={point.month} onClick={() => openMonthRecords(point.month)} aria-label={`查看 ${point.month} 的收入与成本流水`}><span className="trend-columns"><i style={{ height: `${point.revenue / maxTrend * 100}%` }} /><i style={{ height: `${point.cost / maxTrend * 100}%` }} /></span><em>{point.month.slice(5)}月</em></button>)}</div></section><section className="analysis-hidden-cost-card"><div className="analysis-hidden-cost-head"><div><span>行业隐形成本</span><h2>{template.label}潜在漏损</h2></div><strong title={yuan(hiddenCostEstimateTotal)}>{formatMoneyCompact(hiddenCostEstimateTotal)}<small>基准估算</small></strong></div>{visibleHiddenCosts.length ? <><div className="analysis-hidden-cost-list">{visibleHiddenCosts.slice(0, 3).map((item) => <div key={item.key}><span><b>{item.label}</b><em>{`${yuan(item.base)} 基数 × ${(item.rate * 100).toFixed(item.rate * 100 < 10 ? 1 : 0)}%`}</em></span><strong>{yuan(item.estimate)}</strong><small>{item.tip}</small></div>)}</div><div className="analysis-hidden-cost-foot"><span>仅作复核，不计入正式利润。</span><button onClick={() => openMonthRecords(period)}>核对流水 <ChevronRight size={15} /></button></div></> : <button className="analysis-hidden-cost-empty" onClick={openNewRecord}><span>暂无可估算基数</span><b>补录收入后自动生成</b><ChevronRight size={16} /></button>}</section></div>}</section>
    </div>;
    const priority = costSummary.priority!;
    const priorityCopy = priority?.reason === "increase" ? `较上期增加 ${yuan(priority.delta ?? 0)}` : `占本期成本 ${priority?.share || 0}%`;
    return <div className="prototype-analysis ledger-page-shell" data-chart-role={chartRole("analysis").primary}><section className="prototype-analysis-title ledger-page-heading"><h1>成本分析 · {period.replace("-", " 年 ")} 月</h1></section><section className="analysis-cost-overview ledger-surface"><div className="analysis-card-head"><h2>本期成本结论</h2><div className="segment-control"><button className={analysisPeriod === "current" ? "active" : ""} onClick={() => setAnalysisPeriod("current")}>本月</button><button className={analysisPeriod === "last" ? "active" : ""} onClick={() => setAnalysisPeriod("last")}>上月</button></div></div>{costSummary.totalCost > 0 ? <><div className="analysis-cost-total"><span>已入账总成本</span><strong>{yuan(costSummary.totalCost)}</strong><em className={costSummary.delta > 0 ? "up" : costSummary.delta < 0 ? "down" : "flat"}>{costSummary.deltaRate === null ? "暂无上期对比" : `${costSummary.delta >= 0 ? "较上期 +" : "较上期 "}${yuan(costSummary.delta)} · ${Math.abs(costSummary.deltaRate)}%`}</em>{costSummary.costCredits > 0 && <small className="analysis-cost-credit-note">已扣除成本回冲 {yuan(costSummary.costCredits)}；正向成本 {yuan(costSummary.grossCost)}</small>}</div><div className="analysis-cost-kpis"><span><em>最大成本项</em><b>{costSummary.structure[0]?.label || "—"}</b></span><span><em>成本占比</em><b>{costSummary.structure[0]?.share || 0}%</b></span><span className={costSummary.costCredits > 0 ? "cost-credit" : undefined}><em>{costSummary.costCredits > 0 ? "成本回冲" : "成本项目"}</em><b>{costSummary.costCredits > 0 ? `−${yuan(costSummary.costCredits)}` : `${costSummary.structure.length} 项`}</b></span></div></> : <button className="analysis-no-revenue" onClick={openNewRecord}><span>本期尚未录入成本</span><b>补录成本后生成成本结论</b><ChevronRight size={16} /></button>}</section>{priority && <button className="analysis-priority-action ledger-surface" onClick={() => { setRecordSearch(priority.label); setRecordMonth(period); goSub("records"); }}><span><em>优先处理</em><b>{priority.reason === "increase" ? `${priority.label}上升较快` : `${priority.label}占用最多成本`}</b><small>{priorityCopy} · 点击核对流水</small></span><strong>{yuan(priority.amount)}</strong><ChevronRight size={17} /></button>}<section className="analysis-cost-structure ledger-surface"><div className="analysis-card-head"><h2>成本结构</h2><span>点击查看流水</span></div><div className="analysis-cost-list">{costSummary.structure.map((item, index) => <button key={item.key} onClick={() => { setRecordSearch(item.label); setRecordMonth(period); goSub("records"); }}><span className="cost-rank">{index + 1}</span><span className="cost-name"><b>{item.label}</b><i><strong style={{ width: `${item.share}%` }} /></i></span><span className="cost-value"><b>{yuan(item.amount)}</b><em className={item.delta > 0 ? "up" : item.delta < 0 ? "down" : "flat"}>{item.deltaRate === null ? "新发生" : `${item.delta >= 0 ? "↑" : "↓"}${Math.abs(item.deltaRate)}%`}</em></span><ChevronRight size={15} /></button>)}</div>{!costSummary.structure.length && <div className="analysis-empty-copy">当前没有可展示的成本分类。</div>}</section><section className="analysis-more ledger-surface"><button className="analysis-more-trigger" type="button" aria-expanded={analysisDetailsOpen} onClick={() => setAnalysisDetailsOpen((value) => !value)}><span><b>趋势与风险复核</b><em>查看利润、月度趋势和行业漏损估算</em></span><ChevronDown size={18} /></button>{analysisDetailsOpen && <div className="analysis-more-content"><section className="analysis-waterfall-card"><div className="analysis-card-head"><h2>利润与成本</h2></div>{hasRevenue ? <ProfitWaterfall revenue={analysisRevenue} cogs={analysisCogs} expenses={analysisExpenses} profit={periodView.totals.operatingProfit} onSelect={(key) => { if (key === "cogs" || key === "expenses") { setRecordFilter("expense"); setRecordMonth(period); goSub("records"); } else if (key === "revenue") { setRecordFilter("income"); setRecordMonth(period); goSub("records"); } else goSub("reports"); }} /> : <button className="analysis-no-revenue" onClick={openNewRecord}><span>本期尚未录入销售收入</span><b>补录收入后生成利润图</b><ChevronRight size={16} /></button>}</section><section className={`analysis-trend-card ${sparseTrend ? "sparse" : ""}`}><div className="analysis-card-head"><h2>近 6 月收入与成本</h2><div className="chart-legend"><span><i className="revenue" />收入</span><span><i className="cost" />成本</span></div></div><div className="trend-chart">{trend.map((point) => <button type="button" key={point.month} onClick={() => openMonthRecords(point.month)} aria-label={`查看 ${point.month} 的收入与成本流水`}><span className="trend-columns"><i style={{ height: `${point.revenue / maxTrend * 100}%` }} /><i style={{ height: `${point.cost / maxTrend * 100}%` }} /></span><em>{point.month.slice(5)}月</em></button>)}</div></section><section className="analysis-hidden-cost-card"><div className="analysis-hidden-cost-head"><div><span>行业隐形成本</span><h2>{template.label}潜在漏损</h2></div><strong title={yuan(hiddenCostEstimateTotal)}>{formatMoneyCompact(hiddenCostEstimateTotal)}<small>基准估算</small></strong></div>{visibleHiddenCosts.length ? <><div className="analysis-hidden-cost-list">{visibleHiddenCosts.slice(0, 3).map((item) => <div key={item.key}><span><b>{item.label}</b><em>{`${yuan(item.base)} 基数 × ${(item.rate * 100).toFixed(item.rate * 100 < 10 ? 1 : 0)}%`}</em></span><strong>{yuan(item.estimate)}</strong><small>{item.tip}</small></div>)}</div><div className="analysis-hidden-cost-foot"><span>仅作复核，不计入正式利润。</span><button onClick={() => openMonthRecords(period)}>核对流水 <ChevronRight size={15} /></button></div></> : <button className="analysis-hidden-cost-empty" onClick={openNewRecord}><span>暂无可估算基数</span><b>补录收入后自动生成</b><ChevronRight size={16} /></button>}</section></div>}</section></div>;
  }
  ...
  function AnalysisPage() {
    ...
    const periodView = book.getPeriodView(period);
    const previousView = book.getPeriodView(book.previousPeriod(period));
    const structureComparison = buildCostStructureComparison(periodView.totals.categoryTotals, previousView.totals.categoryTotals);
    ...
    const openSupplierRecords = (supplierId: string) => { setRecordFilter("expense"); setRecordMonth(period); setRecordSearch(""); setRecordSupplierFilter(supplierId); goSub("records"); };
    const openStructureRecords = (_key: string, label: string) => { setRecordSearch(label); setRecordMonth(period); goSub("records"); };
    return <><AnalysisPageContent /><section className="analysis-supplement ledger-page-shell" aria-label="成本对照与供应商排行"><section className="analysis-structure-compare ledger-surface" data-chart-template="grouped-bars"><div className="analysis-card-head"><div><h2>本期 / 上期结构</h2><span>正向成本占比</span></div><div className="analysis-grouped-legend" aria-label="图例"><span><i className="current" />本期</span><span><i className="previous" />上期</span></div></div><StructureComparisonGroupedBars items={structureComparison} onSelect={openStructureRecords} /></section><section className="analysis-supplier-rank ledger-surface" data-chart-template="rank"><div className="analysis-card-head"><h2>供应商成本排行</h2><span>已关联支出</span></div>{supplierRankings.length ? <div className="analysis-supplier-list template-supplier-lollipop">{supplierRankings.map((item, index) => <button key={item.supplierId} onClick={() => openSupplierRecords(item.supplierId)}><i>{index + 1}</i><span><b>{item.label}</b><em>{item.entryCount} 笔已入账支出 · 占已关联成本 {item.share}%</em><i className="supplier-lollipop" aria-hidden="true"><em style={{ width: `${item.amount / supplierRankMax * 100}%` }} /></i></span><strong>{yuan(item.amount)}</strong><ChevronRight size={15} /></button>)}</div> : <button className="analysis-supplier-empty" onClick={() => { setRecordType("expense"); openNewRecord(); }}><span><b>暂无已关联供应商成本</b><em>记录支出时关联供应商，即可在此查看排行。</em></span><ChevronRight size={16} /></button>}</section></section></>;
  ...
  function IndirectCostsPage() {
    ...
    return <div className="indirect-cost-page ledger-page-shell"><section className="sub-intro compact"><span>{template.label} · 项目完全成本</span><h1>本期成本设置</h1><p>把房租、水电、摊位费等录成一笔成本，再决定是否分到项目。行业漏损估算仍只作风险提醒，不会混入这里。</p></section><section className="indirect-cost-summary ledger-surface"><div><em>本月已分摊</em><b>{yuan(allocatedTotal)}</b><small>{indirectCostPools.filter((pool) => pool.allocationMode === "allocated").length} 笔成本池</small></div><div><em>暂不分摊</em><b>{yuan(unallocatedTotal)}</b><small>仍会计入实际经营费用</small></div></section>{cards.length === 0 ? <section className="indirect-cost-empty ledger-surface"><b>先建立要承担成本的项目</b><p>新增成本卡后，才能把间接成本分配到商品或服务项目。</p><button onClick={openNewCard}><Plus size={16} />新增成本卡</button></section> : <form key={editingIndirectPool?.id || "new"} className="indirect-cost-form ledger-surface" onSubmit={saveIndirectPool}><div className="analysis-card-head"><h2>{editingIndirectPool ? "编辑本期成本" : "新增本期成本"}</h2>{editingIndirectPool ? <button type="button" onClick={() => { setIndirectCostEditId(null); setIndirectAllocationMode("allocated"); setIndirectAllocationMethod("units"); }}>取消编辑</button> : <span>保存后可随时切换是否摊销</span>}</div><div className="form-two-col"><label>费用类型<select name="kind" defaultValue={editingIndirectPool?.kind || "rent"} onChange={(event) => { const select = event.currentTarget.form?.elements.namedItem("categoryKey") as HTMLSelectElement | null; if (select) select.value = categoryForKind(event.target.value as IndirectCostKind); }}><option value="rent">房租</option><option value="utilities">水电物业</option><option value="stall_fee">摊位费</option><option value="base_labor">基础人工</option><option value="depreciation">设备折旧</option><option value="custom">其他间接成本</option></select></label><label>本期金额<div className="amount-input"><span>¥</span><input name="amount" type="number" min="0.01" step="0.01" inputMode="decimal" defaultValue={editingIndirectPool ? (editingIndirectPool.amountFen / 100).toFixed(2) : ""} placeholder="例如：1800" required /></div></label><label>发生日期<input name="date" type="date" defaultValue={today} /></label><label>计入分类<select name="categoryKey" defaultValue={editingIndirectPool?.categoryKey || categoryForKind("rent")}>{categories.map((category) => <option key={category.key} value={category.key}>{category.label}</option>)}</select></label></div><label>备注名称<input name="name" defaultValue={editingIndirectPool?.name || ""} placeholder="例如：8 月门店房租" /></label><section className="indirect-source-choice"><span>这笔钱是否已经发生？</span><div><label><input type="radio" name="source" value="actual" defaultChecked={!editingIndirectPool || editingIndirectPool.source === "actual"} />已发生，保存为经营费用</label><label><input type="radio" name="source" value="planned" defaultChecked={editingIndirectPool?.source === "planned"} />计划费用，仅作成本测算</label></div></section><section className="allocation-switch"><div><span><b>一键摊销到项目</b><em>把这笔费用纳入项目完全成本与后续定价。</em></span><div className="allocation-mode-options"><label className={indirectAllocationMode === "allocated" ? "selected" : ""}><input type="radio" name="allocationMode" value="allocated" checked={indirectAllocationMode === "allocated"} onChange={() => setIndirectAllocationMode("allocated")} />一键摊销</label><label className={indirectAllocationMode === "unallocated" ? "selected" : ""}><input type="radio" name="allocationMode" value="unallocated" checked={indirectAllocationMode === "unallocated"} onChange={() => setIndirectAllocationMode("unallocated")} />暂不摊销</label></div></div>{indirectAllocationMode === "allocated" && <><div className="allocation-rule"><span>怎么分给项目？</span><select name="allocationMethod" value={indirectAllocationMethod} onChange={(event) => setIndirectAllocationMethod(event.target.value as IndirectCostAllocationMethod)}>{(Object.keys(indirectCostAllocationMethodLabel) as IndirectCostAllocationMethod[]).map((method) => <option value={method} key={method}>{indirectCostAllocationMethodLabel[method]}</option>)}</select></div><p className="allocation-rule-note">{indirectAllocationMethod === "units" ? "卖得更多的项目多承担；本期没有销量时不会自动猜测分配。" : indirectAllocationMethod === "revenue" ? "营收更高的项目多承担；适合房租、门店基础费用。" : indirectAllocationMethod === "equal" ? "勾选的项目平均承担；适合项目价值相近的情况。" : "请为勾选项目填写比例，合计必须为 100%。"}</p><div className="allocation-targets">{cards.map((card) => <label key={card.id}><input type="checkbox" name={`target-${card.id}`} defaultChecked={!editingIndirectPool || editingIndirectPool.targets.some((target) => target.cardId === card.id)} /> <span><b>{card.name}</b><em>{card.kind || template.entityLabel}</em></span>{indirectAllocationMethod === "manual" && <span className="allocation-weight"><input name={`weight-${card.id}`} type="number" min="0" max="100" step="0.1" defaultValue={editingIndirectPool?.targets.find((target) => target.cardId === card.id)?.manualWeight ?? (100 / Math.max(cards.length, 1)).toFixed(1)} /><i>%</i></span>}</label>)}</div></>}</section><button className="fixed-primary form-save" type="submit"><Check size={18} />{indirectAllocationMode === "allocated" ? "保存并一键摊销" : "保存为暂不摊销费用"}</button></form>}<section className="indirect-cost-list ledger-surface"><div className="analysis-card-head"><h2>本月成本池</h2><span>{indirectCostPools.length} 笔</span></div>{indirectCostPools.map((pool) => { const allocations = indirectCostAllocations.filter((item) => item.poolId === pool.id); return <article key={pool.id}><div><span><b>{pool.name}</b><em>{indirectCostKindLabel[pool.kind]} · {pool.source === "actual" ? "已入账" : "计划"} · {pool.allocationMode === "allocated" ? indirectCostAllocationMethodLabel[pool.allocationMethod] : "暂不摊销"}</em></span><strong>{yuan(pool.amountFen / 100)}</strong></div>{pool.allocationMode === "allocated" ? <p>已分给 {allocations.length} 个项目{allocations.length ? ` · ${allocations.map((item) => cards.find((card) => card.id === item.cardId)?.name || "项目").join("、")}` : "；本期缺少所需销量或营收基数，等待数据后自动计算"}</p> : <p>已保留为经营费用，尚未增加任何项目的单位成本。</p>}<footer><button onClick={() => { setIndirectCostEditId(pool.id); setIndirectAllocationMode(pool.allocationMode); setIndirectAllocationMethod(pool.allocationMethod); window.scrollTo({ top: 0, behavior: "smooth" }); }}>编辑</button><button onClick={() => { book.setIndirectCostAllocationMode(pool.id, pool.allocationMode === "allocated" ? "unallocated" : "allocated"); notify(pool.allocationMode === "allocated" ? "已停止项目摊销，原始经营费用保持不变" : "已重新启用项目摊销；将按原规则计算"); }}>{pool.allocationMode === "allocated" ? "暂不摊销" : "一键摊销"}</button><button className="danger" onClick={() => { if (!window.confirm("确认删除这项分摊设置？原始经营费用会保留在流水中。")) return; book.removeIndirectCostPool(pool.id); notify("已删除分摊设置，原始费用未改动"); }}>删除设置</button></footer></article>; })}{!indirectCostPools.length && <div className="analysis-hidden-cost-empty"><span>还没有间接成本池</span><b>从房租、水电、摊位费或基础人工开始记录</b></div>}</section><section className="indirect-cost-boundary"><CircleAlert size={17} /><p><b>口径说明：</b>已发生费用会进入经营利润；“一键摊销”只把它同步显示到项目完全成本和后续定价，不重复扣减利润，也不会改写历史订单。</p></section></div>;
  }
  ...
  const tabs: { id: TabId; label: string; icon: LucideIcon }[] = [{ id: "home", label: "经营", icon: HomeIcon }, { id: "orders", label: "订单", icon: ReceiptText }, { id: "cards", label: cardCopy.tabLabel, icon: PackageOpen }, { id: "analysis", label: "分析", icon: BarChart3 }, { id: "profile", label: "我的", icon: WalletCards }];
