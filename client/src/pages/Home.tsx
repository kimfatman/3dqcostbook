...
export default function Home() {
  ...
  function HomePage() {
    ...
    const operatingLabel = totals.operatingProfit >= 0 ? "预计经营利润" : "预计经营亏损";
    return <>
      ...
      <EquationResult firstLabel="净营收" firstValue={yuan(totals.revenue)} secondLabel="经营成本" secondValue={yuan(totals.totalCost)} resultLabel={operatingLabel} resultValue={yuan(Math.abs(totals.operatingProfit))} detail={`毛利率 ${totals.grossMarginRate}% · 经营利润率 ${totals.operatingMarginRate}% · 预算已用 ${totals.budgetUsed.toFixed(1)}%`} />
  ...
  function AnalysisPage() {
    ...
    const hasProfit = periodView.totals.operatingProfit >= 0;
    return <><section className="screen-title"><span>经营分析 · {period.replace("-", " 年 ")} 月</span><h1>先看结果，再看原因</h1><p>所有判断均来自当前行业已过账交易，不使用比例缩放。</p></section><div className="segment-control"><button className={analysisPeriod === "current" ? "active" : ""} aria-pressed={analysisPeriod === "current"} onClick={() => setAnalysisPeriod("current")}>本月</button><button className={analysisPeriod === "last" ? "active" : ""} aria-pressed={analysisPeriod === "last"} onClick={() => setAnalysisPeriod("last")}>上月</button></div><EquationResult firstLabel="净营收" firstValue={yuan(periodView.totals.revenue)} secondLabel="经营成本" secondValue={yuan(periodCost)} resultLabel={hasProfit ? "经营利润" : "经营亏损"} resultValue={yuan(Math.abs(periodView.totals.operatingProfit))} detail={`毛利率 ${periodView.totals.grossMarginRate}% · 预算已用 ${periodView.totals.budgetUsed.toFixed(1)}%`} /><section className="analysis-judgment"><span>为什么</span><b>{primaryReason ? `${primaryReason.label}占本期成本 ${Math.round(primaryReason.amount / Math.max(periodCost, 1) * 100)}%` : "等待更多成本数据"}</b><p>{primaryReason ? `${primaryReason.label}为当前第一成本来源。${hasProfit ? "利润为正，优先保持可控成本并复核异常上升项。" : "经营结果为负，建议先核对该项的采购、价格或用量。"}` : "先完成第一笔真实交易记录，系统会生成成本原因和行动建议。"}</p><button onClick={() => primaryReason ? (setRecordSearch(primaryReason.label), goSub("records")) : goSub("record")}>{primaryReason ? `复核${primaryReason.label}` : "＋ 记录第一笔成本"}<ChevronRight size={15} /></button></section>
