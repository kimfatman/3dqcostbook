...
export default function Home() {
  ...
  function IndustryPage() {
    return <><section className="sub-intro"><span>选择你的经营方式</span><h1>一套账本，<br />按行业展开。</h1><p>历史流水、成本卡和月报保持原行业口径；切换后新记录按目标行业归集。</p></section><div className="industry-picker">{Object.values(industryTemplates).map((item) => { const Icon = iconByIndustry[item.id]; const chosen = item.id === selectedIndustry; return <button key={item.id} className={chosen ? "chosen" : ""} aria-pressed={chosen} onClick={() => setSelectedIndustry(item.id)}><span className="picker-icon"><Icon size={21} /></span><span><b>{item.label}</b><em>{item.descriptor}</em><small>{item.categories.slice(0, 2).map((category) => category.label).join(" / ")}</small></span>{chosen && <i>已选</i>}</button>; })}</div><div className="switch-note"><CircleAlert size={15} />切换后会创建目标行业分类；历史账本、成本卡和报告不会删除。</div><button className="fixed-primary" onClick={applyIndustry}>使用{industryTemplates[selectedIndustry].label}模板</button></>;
  }
