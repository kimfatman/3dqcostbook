...
export const navigationSubPages: Exclude<NavigationSubPage, null>[] = ["notifications", "industry", "records", "record", "recordDetail", "cards", "cardDetail", "cardForm", "bomForm", "pricing", "budget", "healthSettings", "salesTargets", "reports", "reportDetail", "suppliers", "supplierForm", "categories", "categoryForm", "orders", "orderForm", "orderDetail", "refundForm", "skus", "indirectCosts", "profileSettings", "appearance", "avatarStyle", "storeBrand"];
const parentTabBySubPage: Record<Exclude<NavigationSubPage, null>, NavigationTab> = { notifications: "home", industry: "profile", records: "home", record: "home", recordDetail: "home", cards: "cards", cardDetail: "cards", cardForm: "cards", bomForm: "cards", pricing: "cards", budget: "profile", healthSettings: "analysis", salesTargets: "analysis", reports: "analysis", reportDetail: "analysis", suppliers: "profile", supplierForm: "profile", categories: "profile", categoryForm: "profile", orders: "orders", orderForm: "orders", orderDetail: "orders", refundForm: "orders", skus: "cards", indirectCosts: "analysis", profileSettings: "profile", appearance: "profile", avatarStyle: "profile", storeBrand: "profile" };

export function readNavigationState(search: string): NavigationState {
  const params = new URLSearchParams(search);
  const screen = params.get("screen");
  const recordContext: RecordNavigationContext = { filter: params.get("filter") || undefined, month: params.get("month") || undefined, query: params.get("q") || undefined, categoryKey: params.get("category") || undefined, skuId: params.get("sku") || undefined };
...
export function navigationSearch(state: NavigationState): string {
  ...
  if (["records", "record", "recordDetail"].includes(screen)) {
    ...
    if (state.recordContext?.categoryKey) params.set("category", state.recordContext.categoryKey);
    if (state.recordContext?.skuId) params.set("sku", state.recordContext.skuId);
  }
