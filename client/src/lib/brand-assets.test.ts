...
describe("算得清品牌资产索引", () => {
  it("覆盖设计包全部 16 个视觉 PNG，并校验 1 个资产清单 JSON", () => {
    expect(brandAssetList).toHaveLength(16);
  ...
  it("按场景区分品牌参考与运营宣传资产", () => {
    expect(brandAssetsByKind("brand").map((item) => item.id)).toEqual(["vi_master_board", "logo_mark", "splash_blue"]);
    ...
    expect(brandAssets.logoMark.path).toContain("SDQ_Logo_Mark.png");
  });
});
