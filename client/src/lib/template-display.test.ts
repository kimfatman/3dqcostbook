...
describe("行业模板成本卡显示名称", () => {
  it("为五类行业提供与实体名称一致的成本卡页面与底部导航名称", () => {
    const expected: Record<IndustryId, { tabLabel: string; title: string; unitProfitLabel: string; description: string; imageManagementCopy: string }> = {
      canteen: { tabLabel: "菜品", title: "菜品成本卡", unitProfitLabel: "单份利润", description: "管理菜品成本卡的成本与定价，提升单份利润", imageManagementCopy: "菜品图片仅在菜品成本卡详情中管理。" },
      retail: { tabLabel: "商品", title: "商品成本卡", unitProfitLabel: "单件利润", description: "管理商品成本卡的成本与定价，提升单件利润", imageManagementCopy: "商品图片仅在商品成本卡详情中管理。" },
      ecommerce: { tabLabel: "商品", title: "商品成本卡", unitProfitLabel: "单件利润", description: "管理商品成本卡的成本与定价，提升单件利润", imageManagementCopy: "商品图片仅在商品成本卡详情中管理。" },
      beauty: { tabLabel: "项目", title: "服务项目成本卡", unitProfitLabel: "单次利润", description: "管理服务项目成本卡的成本与定价，提升单次利润", imageManagementCopy: "服务项目图片仅在服务项目成本卡详情中管理。" },
      stall: { tabLabel: "货品", title: "货品成本卡", unitProfitLabel: "单件利润", description: "管理货品成本卡的成本与定价，提升单件利润", imageManagementCopy: "货品图片仅在货品成本卡详情中管理。" },
    };
  ...
  it("让首页、订单、成本卡、SKU、资料、预算和分析页统一读取当前行业的实体、单位和分类", () => {
    (Object.keys(industryTemplates) as IndustryId[]).forEach(id => {
      ...
      expect(audit.cards).toMatchObject({ title: `${template.entityLabel}成本卡`, formulaLabel: template.formulaLabel });
      expect(audit.sku).toMatchObject({ title: `SKU ${template.entityLabel}成本卡`, unitLabel: template.unitLabel });
