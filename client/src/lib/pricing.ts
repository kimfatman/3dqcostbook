 ...
 * 价格 = (单位完全成本 + 单件履约费用) / (1 - 平台费率 - 目标贡献毛利率)，再按指定步长向上取整。
 */
...
const roundUp = (value: number, step: number) => money(Math.ceil((value - 1e-9) / step) * step);
...
export function quotePrice(input: PricingInput): PriceQuote {
  ...
  const roundingStep = input.roundingStep > 0 ? input.roundingStep : 0.01;
  ...
  const price = roundUp(rawPrice, roundingStep);
...
export function breakEvenPrice(unitCost: number, platformRatePct: number, fulfillmentCost: number, roundingStep: number): PriceQuote {
  return quotePrice({ unitCost, platformRatePct, fulfillmentCost, targetContributionMarginPct: 0, roundingStep });
}
