...
当前尚未配置自动离机备份。用户于 2026-08-21 明确选择暂不启用自动备份或腾讯云 COS，因此当前仅依赖服务器本地持久化；单台云服务器磁盘故障、误删或账户事故无法保证恢复。后续启用时，应优先建立每日 MySQL 备份，并将加密备份同步至腾讯云 COS 或另一台独立存储位置；实施前需确认保留周期、COS 存储桶地域和恢复演练频率。
