...
| 全量门禁 | 服务端代理迁移后待重新执行完整门禁与腾讯云生产验收。 |
| 腾讯云构建与迁移 | 之前的浏览器直连版本已验证；服务端代理版本待重新部署。 |
| 服务可用性 | 通过。`https://api.3dq.site/healthz` 返回 `{"status":"ok"}`，`https://app.3dq.site/` 返回 HTTP `200`。 |

构建保留项目既有的主 bundle 大于 500kB 提示；服务端代理迁移后未引入新的本地编译错误。腾讯云生产需要重新部署，以使浏览器不再依赖 CloudBase 自定义安全域名白名单。部署后，为避免对未授权邮箱或手机号发信，仍不自动发送真实验证码；应由真实店主账号手动完成一次邮箱和短信的送达、验证、登录及退出，且不要将验证码或 Key 发送到聊天。
