...
| 腾讯云构建与迁移 | 通过。Docker 编排已将两个 `VITE_CLOUDBASE_*` 变量传入 Vite 构建，并将 `CLOUDBASE_ENV_ID`、`CLOUDBASE_APIKEY` 映射到运行容器；`0005_lush_dexter_bennett.sql` 已成功应用。 |
| 腾讯云运行态 | 通过。应用容器运行，CloudBase 服务器变量存在性检查通过（未输出值），运行 UID 为 `10001`；线上源码与已构建前端均包含验证码入口标记。 |
| 服务可用性 | 通过。`https://api.3dq.site/healthz` 返回 `{"status":"ok"}`，`https://app.3dq.site/` 返回 HTTP `200`。 |

构建保留项目既有的主 bundle 大于 500kB 提示；本轮未引入新的构建错误。腾讯云生产部署与配置映射已验证，但为了避免对未授权邮箱或手机号发信，尚未自动发送真实验证码。下一步应由真实店主账号手动完成一次邮箱和短信的送达、验证、登录及退出；不要将验证码或 Key 发送到聊天。
