...
- **入口**：底部导航「洞察」升级为「经营分析」（决策点 D1），顶部固定全局控件条：行业模板下拉 + 导出PDF报表 + 重置模板。
- **成本分析 Tab**（自上而下 4 卡）：
...
- **D1 入口命名**：底部 Tab 改名为「经营分析」，还是保留「洞察」并新增「经营分析」入口？（PRD 用"经营分析"一级入口）
- **D2 行业模板**：现有 5 套（餐饮/零售/电商/美业/小商贩）与 PRD 7 套（餐饮/烘焙/美业/汽修/鲜果奶茶/加工厂/自定义）如何共存？涉及存量用户行业数据。
- **D3 毛利口径**：单品/套餐毛利是否采用 PRD 纯物料口径（售价−物料−打包），与现有含人工/分摊的 SKU 贡献毛利并存展示？
- **D4 会员权益**：库存对账/台账/PDF 导出的会员门槛如何落地（扩展订阅字段 or 管理后台权益配置）？
- **D5 交付载体**：本次升级先在现有 Web App 落地（建议，小程序复用同一计算引擎），还是直接走微信云开发？
