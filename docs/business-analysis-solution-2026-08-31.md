...
- **D2 行业模板：✅ 已定稿（2026-08-31）**：**PRD 7 套为主推 + 旧 5 套全保留，旧 3 套归入「更多/通用」分组**。
...
- **D3 毛利口径：✅ 已定稿（2026-08-31）**：**双口径并存**——经营分析「毛利核算卡」采用 PRD 纯物料口径（售价−物料−打包，对应餐饮食材成本率 25–30% 红线）；现有含人工/分摊的 SKU 贡献毛利与成本卡口径**原样保留**，不互相覆盖。两处分别标注口径名，避免混淆。
- **D4 会员权益：✅ 已定稿（2026-08-31）**：**一期免费开放全部功能，会员门槛留到二期**。库存对账/每日台账/PDF 导出仍按 PRD 归入二期；二期在现有管理后台/账号体系扩展订阅字段（参考 `auth-admin-scope-plan-2026-08-26.md`），前端读权益状态，未订阅时显示升级引导（锁定态/空态），不阻塞一期功能完整性。
- **D5 交付载体：✅ 已定稿（2026-08-31）**：**先在现有 Web App 落地**，小程序复用同一计算引擎。`business-analysis/` 纯函数层设计为无 UI 依赖，小程序化时按 `wechat-miniapp-foundation.md` 把新域映射为云集合即可；先 Web 后小程序，契合一期快速落地节奏。
