...
**状态：已选择方案 A 并完成开发，待生产登录态验收。**

## 实施更新（2026-08-22）

用户已选择**方案 A：引导式成本池**，并要求每笔费用都能明确选择“**一键摊销**”或“**暂不摊销**”。当前实现提供房租、水电物业、摊位费、基础人工、设备折旧和自定义费用六类成本池；用户可选择已发生费用或计划费用、勾选受影响项目，并按销量、营业额、项目数或自定义比例分配。

实际费用会生成一笔正式经营流水，因此仍按原规则进入利润与预算；计划费用仅进入管理口径的项目完全成本。启用一键摊销后，成本卡与智能定价会显示本期项目分摊；暂不摊销仅停止项目成本分配，不删除原始费用。恢复或撤销摊销只影响后续项目成本与订单，既有订单保留当时冻结的成本快照。

该版本已发布至生产环境；2026-08-22 的服务验证确认 API 健康接口正常、前端 HTTPS 返回 200，应用、反向代理与数据库容器均正常运行。由于成本池会写入用户的真实经营费用，新增房租、水电或摊位费以及后续订单成本快照仍需要由登录用户在生产环境完成冒烟验证。
