...
| 后端 Agent | tRPC/Express、认证、数据库、迁移、安全、部署脚本与服务端测试 | 视觉重构、产品文案大改或未经授权的生产发布 | 统筹/发布 Agent |
...
4. 每次后端改动必须新增或更新 Vitest 回归，并在交接前运行 pnpm check、pnpm test、pnpm build。
...
严禁事项：
1. 不擅自重构 client/** 的视觉或交互；如需要前端契约改动，只做必要类型/调用说明，并要求 UI Agent 实现和复核。
...
2. 提交前在 PR 模板填写“主办 Agent（后端）”任务/会话标识、接口/迁移影响、数据不变量验证、失败/权限边界、测试命令与结果。
